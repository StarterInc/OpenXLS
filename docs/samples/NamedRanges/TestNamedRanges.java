    ...
    public void testit(String argstr){
        ...
        try{
           ...
           // this will throw a CellNotFoundException if the name is not found.
            ...
            io.starter.OpenXLS.util.Logger.log(nand.getName());
            
            NameHandle nand2 = tbo.getNamedRange("nametest7");
            CellHandle[] ch1 = nand2.getCells();
            for(int x = 0;x<ch1.length;x++){
                io.starter.OpenXLS.util.Logger.log(ch1[x].getWorkSheetName() +":"+ ch1[x].getCellAddress() +":"+ ch1[x].getVal());
            }
            nand2.setName("IMPORTANT");
            io.starter.OpenXLS.util.Logger.log(nand2.getName());            
...
//            sheet1.add("Ken", cx[4].getRowNum(), cx[4].getColNum());
            ...
            for(int x = 0;x<ch2.length;x++){
                io.starter.OpenXLS.util.Logger.log(ch2[x].getWorkSheetName() +":"+ ch2[x].getCellAddress() +":"+ ch2[x].getVal());
            }
            io.starter.OpenXLS.util.Logger.log(nand3.getName());        
			...
			try{
				...
				for(int t=0;t<ranges.length;t++){
					io.starter.OpenXLS.util.Logger.log("Got new range:" + ranges[t].toString());
				}
			}catch(Exception e){
