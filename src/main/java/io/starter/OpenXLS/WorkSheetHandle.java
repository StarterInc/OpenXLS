...
public class WorkSheetHandle implements Handle{
     ...
     */
    public void setProtected (boolean protect, String password)
    throws WorkBookException{
    	...
    	if(oldpass!=null){
			if(!oldpass.equals(passcheck)&&
					oldpass!="0000"){
				throw new WorkBookException("Incorrect Password Attempt to Unprotect Worksheet.",WorkBookException.SHEETPROTECT_INCORRECT_PASSWORD);
			}
    	}
    ...
    */
    public void setSelected(boolean b){mysheet.setSelected(b);}
    ...
    */
    ...
    throws ColumnNotFoundException{
        ...
        if(ci == null || !ci.isSingleCol()){
            ...
            }catch(Exception e) {throw new ColumnNotFoundException("Unable to getCol for col number " + clnum + " " + e.toString());}
        }else{
    ...
    */
    ...
    throws RowNotFoundException{
        ...
        if(x == null)throw new RowNotFoundException("Row "+t+
    ...
    */
    public void removeRow(int rownum, boolean shiftrows)
    throws RowNotFoundException{
    	if(shiftrows)
    		removeRow(rownum);
    	else
	...
	*/
	public void removeRowContents(int rownum) 
	throws RowNotFoundException{
...
*/
    public void removeRow(int rownum, int flag) 
    throws RowNotFoundException{
		...
		for (int i= 0; i < charts.size(); i++) {
			...
			while (ii.hasNext()) {
				 io.starter.formats.XLS.charts.Series s= ( io.starter.formats.XLS.charts.Series) ii.next();
    ...
    private void removeCols (int first, int count, boolean shift) {
    	if (first < 0) throw new IllegalArgumentException(
    			"column index must be zero or greater" );
     ...
     */
    @Deprecated
    public void removeCol (String colstr, boolean shiftcols)
    throws ColumnNotFoundException {
...
*/
    ...
    */
    public byte[] getSerialBytes(){
        ...
        try{
            ...
            bufo.flush();
            b = baos.toByteArray();
        }catch(Throwable e){Logger.logWarn("Serializing Sheet: " + this.toString() + " failed: " + e);}
        return b;
    ...
    */
    protected WorkSheetHandle(Boundsheet sht, WorkBookHandle b){
	    ...
	    String shiftRule= (String)System.getProperties().get("io.starter.OpenXLS.WorkSheetHandle.shiftInclusive");
	    ...
	    if (System.getProperty("io.starter.OpenXLS.cacheCellHandles")!=null)
	    	cache= Boolean.valueOf((System.getProperty("io.starter.OpenXLS.cacheCellHandles"))).booleanValue();
    }
    
    /**
     * @return setting on whether to use cache or not
     */
    public boolean getUseCache() { return cache;  }
    ...
    */
    public void setVal(String address, Object val)
    ...
    /**
       ...
       @param address
       @param d
       @exception io.starter.formats.XLS.CellNotFoundException is
       thrown if there is no existing Cell at the specified address.
     */
    public void setVal(String address, double d)
    throws CellNotFoundException, CellTypeMismatchException{
        CellHandle c = this.getCell(address);
        c.setVal(d);
    }
    ...
    */
    public void setVal(String address, String s)
    throws CellNotFoundException, CellTypeMismatchException{
        CellHandle c = this.getCell(address);
        c.setVal(s);
    }
    ...
    */
    ...
    /**
       ...
       @exception io.starter.formats.XLS.CellNotFoundException
       is thrown if there is no existing Cell at the specified address.
     */
    public void setVal(String address, int i)
    throws CellNotFoundException, CellTypeMismatchException{
        CellHandle c = this.getCell(address);
        c.setVal(i);
    }
...
*/    
	...
	*/    
	...
	boolean deleteRow(int rownum, int flag, boolean shiftrows){
		 ...
		 * @return
		 */
		private boolean shiftRow(int rownum,
				Row copyRow,
				int flag,
				boolean shiftrows,
				int offset){
			...
			if (shiftrows && flag!=WorkSheetHandle.ROW_INSERT_NO_REFERENCE_UPDATE){
				...
				//  OpenXLS default behavior is to update one row too high.  If we are using ROW_INSERT, update per excel standard,
				// see TestInsertRows.testUpdateFormulaSettings() for testing
				if(flag==WorkSheetHandle.ROW_INSERT)refUpdateStart++;
				ReferenceTracker.updateReferences(refUpdateStart, offset, this.mysheet, true);	//shift or expand/contract ALL affected references including named ranges
			}
			
			int firstcol = copyRow.getColDimensions()[0];
			Row newRow = mysheet.insertRow(rownum, firstcol, flag, shiftrows);	// shifts rows down and inserts a new row, also shifts shared formula refs (see note below) 			
				...
				for (int i=0;i<copyRowCells.length;i++) {
					...
					if (copyRowCell.getOpcode() == XLSRecord.FORMULA &&
						...
						try{
							...
							ReferenceTracker.adjustFormulaRefs(
									newCellHandle, 
									refMovementRow,	   	 
									refMovementDiff*-1,	 	
									true);								
						    ...
						    if(DEBUGLEVEL > 0)Logger.logWarn("WorkSheetHandle.shiftRow() could not adjust formula references in formula: " +copyCellHandle+" while inserting new row."+ e.toString());
						}
					} else {
	...
	//				handle merged cells -- assemble the newmerges collection for below
					...
					if(oby!=null) { // we have a merge
				        ...
				        if(DEBUGLEVEL >10)Logger.logInfo("WorksheetHandle.insertRow() created new Merge Range: " + newrng);				        
    ...
    */
    public CellHandle add(java.util.Date dt, int row, int col,String fmt){
		...
		if (fmt==null) {
		    ...
		    if(fmt == null || fmt.equals("General"))fmt = "m/d/yy h:mm";
		}
...
public CellHandle[] addValidated(java.util.Date dt, int row, int col,String fmt) throws ValidationException{
    ...
    */
    ...
    /** Returns a CellHandle for working with
	    ...
	    @exception CellNotFoundException is thrown if there is no existing Cell at the specified address.
	*/
	public CellHandle getCell(int row, int col)
	throws CellNotFoundException{
    ...
    /** Returns a CellHandle for working with
        ...
        @exception CellNotFoundException is thrown if there is no existing Cell at the specified address.
    */
    public CellHandle getCell(int row, int col, boolean cache)
    throws CellNotFoundException{
    	CellHandle ret = null;
    	if(cache){
	        int[] rc = {row, col};
    ...
    */
    public void moveCell(CellHandle c, String addr)
	...
	*/
    ...
    public void freezeRow(int row){ 
        if (this.mysheet.getPane()==null) this.mysheet.setPane(null);	// will add new 
    ...
    public int getSplitRowLocation() {
        if (this.mysheet.getPane()==null)return -1;
    ...
    public ConditionalFormatHandle createConditionalFormatHandle(String cellAddress, 
    		String operator, String value1, String value2, String format, 
    ...
    public ValidationHandle createValidationHandle(String cellAddress, byte valueType, byte condition, String errorBoxText, String errorBoxTitle, String promptBoxText, String promptBoxTitle, String firstCondition, String secondCondition) {
        ...
        if(firstCondition != null)d.setFirstCond(firstCondition);
        if(secondCondition!=null)d.setSecondCond(secondCondition);
    ...
    public ValidationHandle[] getAllValidationHandles() {
        if(mysheet.getDvRecs() == null)return new ValidationHandle[0];
	 ...
	 * "{@code io.starter.OpenXLS.csvdelimiter}". 
	 */
	public void readCSV (BufferedReader input)
	throws IOException {
		int rws = 0;
		String field_delimiter = System.getProperty(
