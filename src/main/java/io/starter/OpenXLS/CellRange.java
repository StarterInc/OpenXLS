...
public class CellRange implements Serializable {
    ...
    public CellRange(WorkSheetHandle sht, int[] coords, boolean cb) throws Exception {
        ...
        String c1 = ExcelTools.getAlphaVal(coords[1]) + (coords[0] + 1);
        String c2 = ExcelTools.getAlphaVal(coords[3]) + (coords[2] + 1);
        addr += c1 + ":" + c2;
        this.range = addr;
        this.init();
    }
     ...
     *
     * @param source    the cell whose attributes should be copied or <code>null</code> to
