    ...
    public static int[] getEncompassingDataRange(JSONObject jsonDataRange) {
    	try {
	    	...
	    	for (int i= 0; i < nSeries; i++) {
	    		...
	    		if (!serrange.startsWith(sheet))	continue;
	    		int[] locs= ExcelTools.getRangeRowCol(serrange);
	    		try {
		    		if (locs[0] < retVals[0]) retVals[0]= locs[0];
		    		if (locs[1] < retVals[1]) retVals[1]= locs[1];
		    		if (locs[2] > retVals[2]) retVals[2]= locs[2];
		    		if (locs[3] > retVals[3]) retVals[3]= locs[3];
	    		
		    		String legendrange= series.get("l").toString();
		    		locs= ExcelTools.getRowColFromString(legendrange);
		    		if (locs[0] < retVals[0]) retVals[0]= locs[0];
		    		if (locs[1] < retVals[1]) retVals[1]= locs[1];
		    		if (locs[0] > retVals[2]) retVals[2]= locs[0];
		    		if (locs[1] > retVals[3]) retVals[3]= locs[1];
 ...
 */
    public ChartSeriesHandle getChartSeriesHandle(int idx) {
        ChartSeriesHandle[] series = this.getAllChartSeriesHandles();
        if (series.length>=idx)return series[idx];
        return null;
    }
...
/* TODO: Refactor	    for(int x=0;x<mychart.aivals.size();x++){
	...
	public enum ChartOptions {
		...
		THREED,			/** 3d Effect */
		EXPLODED,		/** Pie, Donut */
		HASLINES,		/** Scatter, Line */
		SMOOTHLINES,	/** Scatter, Line, Radar */
		WIREFRAME,		/** Surface */
		DROPLINES,		/** line, area, stock charts */		
		UPDOWNBARS,		/** line, stock */		
		SERLINES,		/** bar, ofpie */
		HILOWLINES,		/**	line, stock charts */	
		FILLED			/** radar */
	}
...
/* TODO: NEEDED??	public boolean setLocationLocked(String loc, boolean l){
	 ...
	 */
	public ChartSeriesHandle setSeries(int index, String legendCell, String categoryRange, String seriesRange, String bubbleRange) 
	throws CellNotFoundException {
    	String legendText= "";
        try {
            CellHandle ICell = null;
            if (legendCell!=null && !legendCell.equals("")){
            	// 20070707 KSC: allow addition of new cell ranges for legendCell (see OpenXLS.handleChartElement)
            	try {
...
//        if (index < mychart.getAllSeries(nChart).size() && index >= 0) {
    ...
    public ChartSeriesHandle addSeriesRange(CellHandle legendCell, CellRange categoryRange, CellRange seriesRange, CellRange bubbleRange, int nChart) {
    	Series s= null;
    	if (bubbleRange==null)	
    		s =  mychart.addSeries(seriesRange.toString(), categoryRange.toString(), "", legendCell.getWorkSheetName() + "!" + legendCell.getCellAddress(), legendCell.getStringVal(), nChart);
    	else
    		s =  mychart.addSeries(seriesRange.toString(), categoryRange.toString(), bubbleRange.toString(), legendCell.getWorkSheetName() + "!" + legendCell.getCellAddress(), legendCell.getStringVal(), nChart);
	...
	private void addAllSeries(String[] valueRanges, String[] legendCells, String[] bubbleSizeRanges, String categoryRange, int nChart) {
        ...
        try {
        	...
        	this.mychart.getLegend().resetPos(chartMetrics.get("y"), chartMetrics.get("h"), chartMetrics.get("canvash"), legendCells.length);
        } catch (Exception e) { }
		setDimensionsRecord();
		// now add series
    ...
    public ChartSeriesHandle appendRowSeriesToChart(int nChart) {
        ...
        }catch(CellNotFoundException e) {Logger.logErr("ChartHandle.appendRowSeriesToChart: Unable to append series to chart: " +e);}
        return null;
    }
		             ...
		             if (tnm.equals("ChartFonts")) {	
		                ...
		                for(int x = 0;x<xpp.getAttributeCount();x++) {
		                	...
		                	if (attr.equals("name")) {
								...
								if (fColor==0) fColor= 32767;	// necessary?
				        ...
				        map.put(s, this);			        			       
				    }else if (tnm.equals("ChartOptions")) {	// handle chart-type-specific options such as legend options
    	...
    	try {
	        ...
	        while (eventType != XmlPullParser.END_DOCUMENT) {
	        	if(eventType == XmlPullParser.START_TAG) {
		             ...
		             if(tnm.equals("chart")) {   // beginning of DrawingML for a single image or chart; children:  title, plotArea, legend,
/**
 * <sequence>
 ...
 */
						...
						thischart.roundedCorners= xpp.getAttributeValue(0).equals("1");
		            	 ...
		            	 thischart.ooxmlLegend= (io.starter.formats.OOXML.Legend) io.starter.formats.OOXML.Legend.parseOOXML(xpp, lastTag, this.wbh).cloneElement();
		             ...
		             // Parse actual CHART TYPE element (barChart, pieChart, etc.)
					            		...
					            		tnm.equals(OOXMLConstants.threeDchartTypes[ChartHandle.SURFACECHART])) {  // specific chart type-
     ...
     */
    private String getOOXMLName() { return ((OOXMLChart)mychart).getOOXMLName(); }
...
*/	
     ...
     * @deprecated 
	 */
	public void make100PercentStacked(int nChart) {	// bar, col, line, pyramid col + bar, cone col + bar, cylinder col + bar
		int chartType= this.getChartType(nChart);
		this.setChartOption("Stacked", "true", nChart);
	...
	*/
	public String getJSON() {
		...
		try {
			...
			titles.put("XAxis", (type!=ChartConstants.BARCHART)?(this.getXAxisLabel()):this.getYAxisLabel());	// bar axes are reversed ...
			...
			try {	//it's possible to not have any series defined ...
				...
				theChart.put("SeriesFills", chartObjectJSON.getJSONArray("SeriesFills"));	// 20090729 KSC: capture bar colors or fills
			} catch (Exception e) { }
			...
			try {
	        	...
	        	theChart.put("back_grid", mychart.getAxes().getJSON(YAXIS, this.wbh, type, yMax, yMin, nSeries).getJSONObject("back_grid"));
			} catch (Exception e) { }			
			try {  
//				inputJSONObject(theChart, this.getAxis(XAXIS, false).getJSON(this.wbh, type, yMax, yMin, nSeries));
	        	...
	        	theChart.put("back_grid", mychart.getAxes().getJSON(Axis.YAXIS, this.wbh, type, yMax, yMin, nSeries).getJSONObject("back_grid"));
			} catch (Exception e) { }
			...
			*/
			...
			if (this.hasDataLegend()) {
				...
				String l= "";
				for (int i= 0; i < legends.length; i++) l+=legends[i] + ",";
	 ...
	 * @return	JSONArray
	 */
	public JSONArray getSeriesJSON() { return mychart.getSeriesJSON(); }
	/**
...
/*KSC: TAKE OUT JSON STUFF FOR NOW; WILL REFACTOR LATER			try {
	...
	public String getAllSeriesDataJSON() {
		...
		try {
			for (int i= 0; i < series.length; i++) {
				...
				if (i==0) ser.put("c", series[i].getCategoryRange());	// 1 per chart
				s.put(ser);
			}			
		} catch (JSONException e) {
	...
	public String getSVG(double scale) {
		...
		svg.append("<g transform='scale(" + scale  + ")'  onclick='null;' onmousedown='handleClick(evt);' style='width:100%; height:"+ (chartMetrics.get("canvash")*scale) + "px'>");		// scale chart -default scale=1 == 100%
		// JavaScript hooks
...
if (WorkBookFactory.PID==WorkBookFactory.E360) { // save svg for testing purposes
	try {
		java.io.File f= new java.io.File("c:/eclipse/workspace/testfiles/io.starter.OpenXLS/output/charts/FromSheetster.svg");
