 ...
 *
 */
open class InitialContextImpl : Context {
    var nameParser: NameParser = NameParserImpl()
    @JvmField
	protected var env: Hashtable<Comparable<*>, Any>? = null
    // check return... -jm
    @Throws(NamingException::class)
    override fun addToEnvironment(propName: String, propVal: Any): Any {
        return if (env!!.contains(propVal)) {
    ...
    @Throws(NamingException::class)
    override fun createSubcontext(name: Name): Context? { // This method is derived from interface javax.naming.Context
// to do: code goes here
        return null
    }

    @Throws(NamingException::class)
    override fun createSubcontext(name: String): Context? { // This method is derived from interface javax.naming.Context
// to do: code goes here
        return null
    }

    @Throws(NamingException::class)
    override fun destroySubcontext(name: String) { // This method is derived from interface javax.naming.Context
// to do: code goes here
    }

    @Throws(NamingException::class)
    override fun destroySubcontext(name: Name) { // This method is derived from interface javax.naming.Context
// to do: code goes here
    }

    @Throws(NamingException::class)
    override fun getNameInNamespace(): String? { // This method is derived from interface javax.naming.Context
// to do: code goes here
        return null
    }
    ...
    init {
        if (System.getProperties()[CONTEXT_ID] != null) env = System.getProperties()[CONTEXT_ID] as Hashtable<Comparable<*>, Any>? else {
            val loadme = System.getProperty(LOAD_CONTEXT)
            env = Hashtable() // 20070518 KSC: Moved so gets init even if no LOAD_CONTEXT
            if (loadme != null) {
                if (loadme == "true") { // env = new Hashtable(); KSC: See above
