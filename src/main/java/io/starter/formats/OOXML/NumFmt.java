...
public class NumFmt implements OOXMLElement{
	...
	public static String getOOXML(Xf xf) {
		...
		if (xf.getIfmt()>FormatConstants.BUILTIN_FORMATS.length) {	// only input user defined formats ...				
			...
			if (s!=null)  s= StringTool.replaceText(s, "\"", "&quot;"); // replace internal quotes   // 1.6 only s= s.replace('"', "&quot;"); // replace internal quotes
			ooxml.append("<numFmt numFmtId=\"" + xf.getIfmt()+ "\" formatCode=\"" + s + "\"/>");  ooxml.append("\r\n");			
		}
		return ooxml.toString();
	}
}
