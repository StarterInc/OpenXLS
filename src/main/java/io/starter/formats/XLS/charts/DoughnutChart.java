...
public class DoughnutChart extends ChartType {
	...
	public String getSVG(HashMap<String, Double> chartMetrics, HashMap<String, Object>axisMetrics, ChartSeries s) {
		...
		for (int i= n-1; i >= 0; i--) {	// for each series 				
			...
			svg.append("<circle "+getScript("")+" cx='" + centerx + "' cy='" + centery + "' r='" + r + "' "+getStrokeSVG(2, this.getDarkColor())+" fill='none'/>\r\n");
			double x= centerx+r;
			double y= centery;
			String path= "";
			double percentage= 0;				 
		...
		svg.append("<circle "+getScript("")+" cx='" + centerx + "' cy='" + centery + "' r='" + r+ "' " + getStrokeSVG(2, this.getDarkColor()) +" fill='white'/>\r\n");
		svg.append("</g>\r\n");
	...
	public StringBuffer getOOXML(String catAxisId, String valAxisId, String serAxisId) {
		...
		cooxml.append("<c:doughnutChart>");	cooxml.append("\r\n");
		// vary colors???
