...
import io.starter.formats.XLS.Label;
import io.starter.formats.XLS.MSODrawing;
import io.starter.formats.XLS.Obj;
 ...
 **/
    ...
    public JSONObject getOptionsJSON() { return null; }
    
	/** 
	 * gets the chart-type specific OOXML representation (representing child element of plotArea element)
	 * @return 
	 */
    public StringBuffer getOOXML(String catAxisId, String valAxisId, String serAxisId) { return null; }
	 ...
	 */
	public int getBarShape() { return SHAPEDEFAULT; }
	...
	public void WriteMainChartRecs(String fName) {
		class util {
			public void writeRecs(BiffRec b, BufferedWriter writer, int level) throws IOException {
				...
				if (b instanceof io.starter.formats.XLS.charts.SeriesText)
					writer.write("\t[" + ((io.starter.formats.XLS.charts.SeriesText) b).toString()+ "]");
				...
				if (((GenericChartObject) b).chartArr.size() > 0) {
					ArrayList<io.starter.formats.XLS.XLSRecord> chartArr= ((GenericChartObject) b).chartArr; 
		...
		try {
			...
			writer.newLine();
			ArrayList<io.starter.formats.XLS.XLSRecord> chartArr= this.cf.chartArr;
			for (int i= 0; i < chartArr.size(); i++) {
