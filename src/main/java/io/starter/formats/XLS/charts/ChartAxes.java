 ...
 * Often the axes of the primary axis group are displayed to the left and bottom sides of the plot area, while axes of the secondary axis group are displayed on the right and top sides of the plot area.
    ...
    public JSONObject getJSON(int axisType, io.starter.OpenXLS.WorkBookHandle wbh, int chartType, double yMax, double yMin, int nSeries) {
	...
	public HashMap<String, Object> getMetrics(int charttype, HashMap<String, Double> chartMetrics, float[] plotcoords, Object[] categories) {
		...
		if (!this.getTitle(XAXIS).equals("")) {
			io.starter.formats.XLS.Font ef= this.getTitleFont(XAXIS);
		...
		if (this.hasAxis(YAXIS)) {
			...
			if (!title.equals("")) {
    			io.starter.formats.XLS.Font ef= this.getTitleFont(YAXIS);
    			java.awt.Font f= new java.awt.Font(ef.getFontName(), ef.getFontWeight(), (int)ef.getFontHeightInPoints());
    			AtomicInteger h= new AtomicInteger(0);
...
/*    			    			
		...
		if (this.hasAxis(XAXIS) && charttype!=RADARCHART && charttype!=RADARAREACHART) { //(Pie, donut, etc. don't have axes labels so disregard
			...
			double width;
			AtomicInteger rot;
			io.starter.formats.XLS.Font lf;
			String pattern;
			if (charttype!=BARCHART) {
				s= categories;
	 ...
	 * @return
	 */
	private double getLabelOffsets(io.starter.formats.XLS.Font lf, double width, Object[] strings, AtomicInteger rot, String pattern, boolean horiz) {
		double retwidth= 0;
