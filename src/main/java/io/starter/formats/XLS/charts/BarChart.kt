...
open class BarChart(charttype: GenericChartObject, cf: ChartFormat, wb: WorkBook) : ChartType(charttype, cf, wb) {
    protected var bar: Bar? = null
     ...
     * can be one of:
     * <br></br>SHAPECOLUMN	default
     * <br></br>SHAPECONEd
     * <br></br>SHAPECONETOMAX
     * <br></br>SHAPECYLINDER
     * <br></br>SHAPEPYRAMID
     * <br></br>SHAPEPYRAMIDTOMAX
     *
     * @return int bar shape
     */
    override val barShape: Int
        get() = cf!!.barShape
    ...
    val optionsXML: String
        get() {
            ...
            if (bar!!.pcOverlap.toInt() != 0)
                sb.append(" Overlap=\"" + bar!!.pcOverlap + "\"")
            if (bar!!.pcGap.toInt() != 50 && bar!!.pcGap.toInt() != 150)
                sb.append(" Gap=\"" + bar!!.pcGap + "\"")
            return sb.toString()
        }

    override// Dojo: Regular, Stacked, Clustered
            /*			String dojoType;
...
*/
    ...
    override fun getSVG(chartMetrics: HashMap<String, Double>, axisMetrics: HashMap<String, Any>, s: ChartSeries): String {
        ...
        var barw = 0.0
        var yfactor = 0.0    //
        barw = h / ((categories!!.size + 1.0) * (n + 1))    // bar width= height/total number of bars+separators
        if (max != 0.0)
            yfactor = w / max        // w/yMax= scale unit

        val rf = 1    //(!yAxisReversed?1:-1);
        val y0 = h + y - barw * 0.5    //start at bottom and work up (unless reversed)
        // for each series
        for (i in 0 until n) {    // each series group i.e. each bar with a different color
            svg.append("<g>\r\n")
            val curseries = series[i] as DoubleArray
            val curranges = s.seriesRanges!![i] as Array<String>
            for (j in curseries.indices) {        // each category (group of series)
                //						double y= y0 - barw*(i+1)*rf - (j*n*barw*1.5)*rf;		// y goes from 1 series to next, corresponding to bar/column color
                val yy = (y0 - barw * (n + 1).toDouble() * 1.1 * j.toDouble() /* each series (bar group) */
                        - barw * (i + 1) /* each bar in the series group */
                        - barw * 1.5 * rf) /* start */
                val ww = yfactor * curseries[j]                            // width of bar -- measure of series
                svg.append("<rect " + ChartType.getScript(curranges[j]) + " fill='" + seriescolors!![i] + "' fill-opacity='1' stroke='black' stroke-opacity='1' stroke-width='1' stroke-linecap='butt' stroke-linejoin='miter' stroke-miterlimit='4'" +
                ...
                if (l != null) {
                    svg.append("<text x='" + (x + ww + 10.0) + "' y='" + (yy + barw * rf) +
