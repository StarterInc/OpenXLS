	...
	*/
    ...
    private int getSeriesNumberFormat() {
    	...
    	if (ai!=null) {
    		...
    		try {
    			io.starter.formats.XLS.formulas.PtgRef p= (io.starter.formats.XLS.formulas.PtgRef) ai.getCellRangePtgs()[0].getComponents()[0];
        ...
        for (int i=0;i<chartArr.size();i++) {
            ...
            if (br.getOpcode()==AI) {
                ...
                if (thisAi.getType() == Ai.TYPE_TEXT) {
                    try {
	                	io.starter.formats.XLS.formulas.Ptg[] p= thisAi.getCellRangePtgs();
	                	return ((io.starter.formats.XLS.formulas.PtgRef)p[0]).getFormattedValue();
                    } catch (Exception e) {}
                    try {
...
//    	}
    ...
    protected static Series getPrototype(String seriesRange, String categoryRange, String bubbleRange, String legendRange, String legendText, ChartType chartobj) {
        ...
        if (legendRange!=null) {
            ...
            try { ai.changeAiLocation("", legendRange/*(seriesText.getWorkSheetName() + "!" + seriesText.getCellAddress())*/);
            } catch (Exception e) { ; } // it's OK to not have a valid range
...
//        parentChart.addAi(ai);        
        ...
        ai.setWorkBook(book);
        ai.setSheet(parentChart.getSheet());
        try { ai.changeAiLocation(ai.toString(), seriesRange);
        } catch (Exception e) { 
...
//        parentChart.addAi(ai);        
        ...
        ai.setWorkBook(book);
        ai.setSheet(parentChart.getSheet());
        try {ai.changeAiLocation(ai.toString(), categoryRange);
        } catch (Exception e) { ; } // it's OK to not have a valid range
        series.addChartRecord(ai);
//        parentChart.addAi(ai);        
        ...
        if(!bubbleRange.equals("")) {
        	try{ ai.changeAiLocation(ai.toString(), bubbleRange);
        	} catch (Exception e) { ; } // it's OK to not have a valid range
            ai.setRt(2);
 ...
 *              if (((BubbleChart) parentChart.getChartObject()).is3d()) {
    ...
    public boolean hasBubbleSizes() { return sdtValBSz > 0; }
 ...
 **/
     ...
     * @return
     */   
    public SpPr getSpPr() { return shapeProps; }
    /**
     ...
     * @return
     */   
    public Marker getMarker() { return m; }
     ...
     * @return
     */   
    public DLbls getDLbls() { return d; }
    ...
    public StringBuffer getValOOXML(String valstr) {
		...
		ooxml.append("<c:"+valstr+">");	ooxml.append("\r\n");
		...
		ooxml.append("<c:numRef>");	ooxml.append("\r\n");		// number reference
   ...
   public StringBuffer getBubbleOOXML(boolean isBubble3d) {
	   ...
	   for (int i=0;i<chartArr.size();i++) {
		...
		ooxml.append("<c:bubbleSize>");	ooxml.append("\r\n");
		ooxml.append("<c:numRef>");	ooxml.append("\r\n");		// number reference
		ooxml.append("<c:f>" + bubbleAi.toString() + "</c:f>");		ooxml.append("\r\n");			
		ooxml.append("<c:numCache>");	ooxml.append("\r\n");
		try {
			CellHandle[] cells= CellRange.getCells(bubbleAi.toString(), parentChart.wbh);
			ooxml.append(getValueRangeOOXML(cells));  ooxml.append("\r\n");
		...
		ooxml.append("</c:bubbleSize>");	ooxml.append("\r\n");
		if (isBubble3d)
			ooxml.append("<c:bubble3D val=\"1\"/>");	ooxml.append("\r\n");
		return ooxml;
   }
...
*/		
       ...
       if (ai!=null) {
    	   ooxml.append("<c:f>" + OOXMLAdapter.stripNonAscii(ai.getDefinition()) + "</c:f>");	ooxml.append("\r\n");
    	   ooxml.append("<c:strCache>");	ooxml.append("\r\n");
    	   ooxml.append("<c:ptCount val=\"1\"/>");	ooxml.append("\r\n");
    	   ooxml.append("<c:pt idx=\"0\">");	ooxml.append("\r\n");
    	   ooxml.append("<c:v>" + OOXMLAdapter.stripNonAscii(txt) + "</c:v>");
    	   ooxml.append("</c:pt>");	ooxml.append("\r\n");
    	   ooxml.append("</c:strCache>");	ooxml.append("\r\n");
    	   ooxml.append("</c:strRef>");	ooxml.append("\r\n");		    	
    	   ooxml.append("</c:tx>");	ooxml.append("\r\n");
   ...
   private static String getValueRangeOOXML(CellHandle[] cells) {
	   ...
	   ooxml.append("<c:ptCount val=\"" + cells.length + "\"/>");	ooxml.append("\r\n");						
	   for (int j= 0; j < cells.length; j++) {
		   ooxml.append("<c:pt idx=\"" + j + "\">");	ooxml.append("\r\n");
		   if (!cells[j].getStringVal().equals("NaN")) {   		
			   ooxml.append("<c:v>" + cells[j].getStringVal() + "</c:v>");	ooxml.append("\r\n");
		   }else  {	// appears that NaN is an invalid entry 
			   ooxml.append("<c:v>0</c:v>");	ooxml.append("\r\n");
		   }
		   ooxml.append("</c:pt>");	ooxml.append("\r\n");
	   }
