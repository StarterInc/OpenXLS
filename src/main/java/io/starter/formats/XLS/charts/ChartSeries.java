...
//	public void setWorkBook(WorkBookHandle wbh) { this.wbh= wbh; }
    ...
    public double[] getMetrics(boolean isDirty) {
    	...
    	io.starter.formats.XLS.Boundsheet sht= parentChart.getSheet();
    	...
    	if (charttype != PIECHART && charttype != DOUGHNUTCHART) {
    		...
    		legends[i] = io.starter.formats.XLS.OOXMLAdapter.stripNonAscii(myseries.getLegendText()).toString();
...
s     * @return
    ...
    public Series getSeries(String seriesName, int nChart) {
        ...
        for (int i=0;i<seriesperchart.size();i++) {
            Series s = (Series)seriesperchart.get(i);
            if (s.getLegendText().equalsIgnoreCase(seriesName))return s;               
         }
        return null;
    }
...
// *******************************************						
			        }else if (tnm.equals("f")) {	// range element  -- legend cell, Cat range, Value range, Bubble data reference 
			       	 	...
			       	 	else if (idx==-1 || ranges[idx].equals("")) { // shoudln't!! can't have a textual refernce in place of a series or cat value (can you?)
			       	 		Logger.logWarn("ChartSeries.parseOOXML: unexpected text value");
			       	 	}	
			        } else if (tnm.equals("numCache") || tnm.equals("strCache") || tnm.equals("multiLvlStrRef")) {	// parent= cat or vals (series values)
			       	 	cache= tnm;
...
//io.starter.toolkit.Logger.log("Truncate list: old range: " + ranges[idx] + " new range: " + cells.getSheet().getQualifiedSheetName() + "!" + ExcelTools.formatLocation(rc));
    ...
    // TODO: finish options
	// TODO: refactor !!!
	public void resetSeriesNumber() { seriesNumber= 0; }
	int seriesNumber= 0;
    public String getOOXML(int ct, boolean isBubble3d, int nChart) {
        ...
        for (int i= 0; i < v.size(); i++) {
    		...
    		ooxml.append("<c:ser>");	ooxml.append("\r\n");
    			ooxml.append("<c:idx val=\"" + seriesNumber + "\"/>");	ooxml.append("\r\n");
    			ooxml.append("<c:order val=\"" + seriesNumber++ + "\"/>");	ooxml.append("\r\n");
    			// Series Legend
				...
				if (s.getMarker()!=null) ooxml.append(s.getMarker().getOOXML());		// only for Radar, Line or Scatter
				if (s.getDPt()!=null) {
