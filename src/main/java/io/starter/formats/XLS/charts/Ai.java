    ...
    public void populateForTransfer(String origSheetName) {
        if ("".equals( boundName )) {
            ...
            for(int t=0;t<expression.size();t++){
                ...
                } else if (p instanceof PtgMemFunc) {	// 20091015 KSC: for non-contiguous ranges
...
//			if(x instanceof SeriesText)return  ((SeriesText)x ).toString();
			...
			if(DEBUGLEVEL>0)Logger.logWarn("Error getting Chart String value: " + e);
	...
	throws WorkSheetNotFoundException{
		...
		for(int t=0;t<expression.size();t++){
			...
			if(p instanceof PtgArea3d){				
				...
				pt.setIxti((short)x);
                pt.addToRefTracker();	
				if(DEBUGLEVEL > 3)Logger.logInfo("Setting sheet reference for: " + pt.toString()  + "  in Ai record.");   
				...
				pr.setIxti((short)x);
                pr.addToRefTracker();	
				if(DEBUGLEVEL > 3)Logger.logInfo("Setting sheet reference for: " + pr.toString()  + "  in Ai record.");   
				// register with the Externsheet reference
...
*/
	public void setExternsheetRef(int oldRef, int newRef)
	throws WorkSheetNotFoundException{
	    for(int t=0;t<expression.size();t++){
	        Ptg p = (Ptg) expression.get(t);   
	        if(p instanceof PtgArea3d){	            
	...
	public void updateSheetRef(String newSheetName, String origWorkBookName) {
		try {
			...
			if (boundXti>-1) {	// has populate for transfer been called - should! 
				int newSheetNum= -1;
				try{
					if (!boundName.equalsIgnoreCase(origSheetName)) {	// Ai reference is on a dfferent sheet, see if it exists in new workbook
				...
				}catch(Exception e){ // 20080123 KSC: if links arent there, fix == try to make an External ref
		            for(int t=0;t<expression.size();t++){
	...
	*/
	...
	public boolean changeAiLocation(Ptg p, String newLoc) {
		...
		for (int i= 0; i<aiLocs.length; i++) {
			try {	// NOTE: Ai has only 1 expression OR 2: 1= PtgMemFunc 2=PtgParen 
				...
				if (expression.get(0) instanceof PtgMemFunc) {	
					try {	// must find particular ptg in PtgMemFunc's subexression to reset
...
//						}
					...
					if (this.getType()==Ai.TYPE_TEXT) {	// must reset text for SeriesText as well
						try {
	...
	*/
	public boolean changeAiLocation(String loc, String newLoc){	   
		...
		try {
			...
			if (ptg!=null) ((PtgRef) ptg).removeFromRefTracker();	
		} catch (Exception e) { 
		}
		if (z==-1 && newLoc.equals(""))	{// no reference -- happens on legends, category ai's ...
	...
	*/
	...
	public void init() {
		...
		pos += cce;
		if(DEBUGLEVEL > 10)Logger.logInfo(this.getName()+ ":" + this.getDefinition());        
	}
