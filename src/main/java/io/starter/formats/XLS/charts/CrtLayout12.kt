 ...
 */
class CrtLayout12 : GenericChartObject(), ChartObject {
    internal var autolayouttype: Byte = 0
     ...
     */
    var layout: Int
        get() = autolayouttype.toInt()
        set(pos) {
            autolayouttype = pos.toByte()
            this.data[16] = (autolayouttype shl 1).toByte()
        }
	...
	*///x and y specify the offset of the top left corner, relative to its default position, as a fraction of the chart area. MUST be greater than or equal to -1.0 and MUST be less than or equal to 1.0. dx and dy specify the width and height, as a fraction of the chart area, MUST be greater than or equal to 0.0, and MUST be less than or equal to 1.0.
    ...
    val coords: FloatArray?
        get() {
            if (wWidthMode.toInt() == 0) {
                return null
            }
            if (wWidthMode.toInt() == 1) {

            }
            if (wWidthMode.toInt() == 2) {
            }
            return null
        }
    ...
    override fun init() {
        super.init()
        val data = this.data
        autolayouttype = (data!![16] shr 1).toByte()
        wXMode = ByteTools.readShort(data[18].toInt(), data[19].toInt())
        wYMode = ByteTools.readShort(data[20].toInt(), data[21].toInt())
        wWidthMode = ByteTools.readShort(data[22].toInt(), data[23].toInt())
        wHeightMode = ByteTools.readShort(data[24].toInt(), data[25].toInt())
        x = ByteTools.eightBytetoLEDouble(this.getBytesAt(26, 8)!!).toFloat()
        y = ByteTools.eightBytetoLEDouble(this.getBytesAt(34, 8)!!).toFloat()
        dx = ByteTools.eightBytetoLEDouble(this.getBytesAt(42, 8)!!).toFloat()
        dy = ByteTools.eightBytetoLEDouble(this.getBytesAt(50, 8)!!).toFloat()
    }
}
