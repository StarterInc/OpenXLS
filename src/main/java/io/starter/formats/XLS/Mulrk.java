...
public final class Mulrk extends io.starter.formats.XLS.XLSRecord implements Mul
{
    /** 
	* serialVersionUID
	*/
	...
	public void init(){
	    ...
	    if(datalen <= 0){
	        ...
	        for(int i=4;i<rkdatax.length;){
	            ...
	            Rk r = new Rk(); 
				r.init(rkd, rw, rkcol++);
	            if(DEBUGLEVEL > 5)Logger.logInfo(" rk@"+(rkcol-1)+":" + r.getStringVal());
	            i += 6;
			...
			if(DEBUGLEVEL > 5)Logger.logInfo("Done adding Rk recs to: " + this.getCellAddress());
	    }            
	}
   ...
   */
    void updateRks(){
        ...
        }catch(IOException a){Logger.logInfo("parsing record continues failed: " + a);}
