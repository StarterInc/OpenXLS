...
public class OOXMLWriter extends OOXMLAdapter implements OOXMLConstants {
    ...
    protected void writeContentType() throws IOException {
        ...
        for (int i = 0; i < mainContentList.size(); i++) {
            ct.append("<Override PartName=\"" + ((String[]) mainContentList.get(i))[0] + "\" ContentType=\"" + getContentType(((String[]) mainContentList.get(i))[1]) + "\"/>");
            ct.append("\r\n");
        }
        for (int i = 0; i < wbContentList.size(); i++) {
            ct.append("<Override PartName=\"" + ((String[]) wbContentList.get(i))[0] + "\" ContentType=\"" + getContentType(((String[]) wbContentList.get(i))[1]) + "\"/>");
            ct.append("\r\n");
        }
        ...
        for (int i = 0; i < sheetsContentList.size(); i++) {
            if (!((((String[]) sheetsContentList.get(i))[1]).equals("printerSettings") ||
                ...
                ct.append("<Override PartName=\"" + ((String[]) sheetsContentList.get(i))[0] + "\" ContentType=\"" + getContentType(((String[]) sheetsContentList.get(i))[1]) + "\"/>");
                ct.append("\r\n");
            }
        }
        for (int i = 0; i < drContentList.size(); i++) {
            if (!((String[]) drContentList.get(i))[1].equals("image"))  /* image files not included in Content_Type - rather, they are handled via <Default Extension> element*/
                ct.append("<Override PartName=\"" + ((String[]) drContentList.get(i))[0] + "\" ContentType=\"" + getContentType(((String[]) drContentList.get(i))[1]) + "\"/>");
            ct.append("\r\n");
        }
            ...
            switch (dataType) {
                case CellHandle.TYPE_STRING:
                    ...
                    if ((s.indexOf("#") == 0)) {   // 20090521 KSC: must test if it's an error string value
                        isErrVal = (Collections.binarySearch(Arrays.asList("#DIV/0!", "#N/A", "#NAME?", "#NULL!", "#NUM!", "#REF!", "#VALUE!"), s.trim()) > -1);
                    }
                    if (!isErrVal) {
                        writer.write(" t=\"s\">");
