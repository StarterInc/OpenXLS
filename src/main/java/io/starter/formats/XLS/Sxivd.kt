...
class Sxivd : XLSRecord(), XLSConstants {

    override fun init() {
        super.init()
        if (DEBUGLEVEL > 3)
            Logger.logInfo("SXIVD - n: " + this.getData()!!.size + " array:" + Arrays.toString(this.getData()))
    }

    override fun toString(): String {
        return "SXIVD - n: " + this.getData()!!.size + " array:" + Arrays.toString(this.getData())
    }
     ...
     * @param fieldNumber
     */
    fun addField(fieldNumber: Int) {
        data = this.getData()
        val b = ByteTools.shortToLEBytes(fieldNumber.toShort())
        data = ByteTools.append(b, data)
    }

    /**
     * for each two-byte pair in an array of [number of items]*2 bytes,
     * //	 * <br></br>specifies either a pivot field index (must less than the total number of fields as specfied by Sxview)
     * or -2 means that the data field is on the row or col axis
     *
     * @param items
     */
    fun setRowOrColItems(items: IntArray) {
        val data = intToByteArray(items)
        this.setData(data)
    }
    ...
    companion object {
        ...
        val prototype: XLSRecord?
            get() {
                val si = Sxivd()
                si.opcode = XLSConstants.SXIVD
                si.setData(byteArrayOf())
                si.init()
                return si
            }

        fun intToByteArray(value: IntArray?): ByteArray? {
            if (value == null) return null
            val b = ByteArray(value.size)
            var j = 0
            for (i in value) {
                b[j++] = i.toByte()
            }
