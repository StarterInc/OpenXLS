 ...
 */
class FeatHeadr : io.starter.formats.XLS.XLSRecord() {
    internal var isf: Short = 0
    internal var grbit = 0
    internal var iprotObject: Boolean = false
    internal var iprotScenario: Boolean = false
    internal var iprotFormatCells: Boolean = false
    internal var iprotFormatColumns: Boolean = false
    internal var iprotFormatRows: Boolean = false
    internal var iprotInsertColumns: Boolean = false
    internal var iprotInsertRows: Boolean = false
    internal var iprotInsertHyperlinks: Boolean = false
    internal var iprotDeleteColumns: Boolean = false
    internal var iprotDeleteRows: Boolean = false
    internal var iprotSelLockedCells: Boolean = false
    internal var iprotSort: Boolean = false
    internal var iprotAutoFilter: Boolean = false
    internal var iprotPivotTable: Boolean = false
    internal var iprotSelUnlockedCells: Boolean = false
     ...
     * default constructor
     */
    private val PROTOTYPE_BYTES = byteArrayOf(0x67, 0x08, // rt or record type
    ...
    override fun init() {
        super.init()
        isf = ByteTools.readShort(this.getData()!![12].toInt(), this.getData()!![13].toInt())
        if (isf.toInt() == 2) {// Enhanced Protection - only option supported for now - read last 4 bytes for enhanced protection settings
     ...
     */
    private fun parseProtectionGrbit() {
        // parse grbit
        iprotObject = grbit and ALLOWOBJECTS == ALLOWOBJECTS.toInt()
        iprotScenario = grbit and ALLOWSCENARIOS == ALLOWSCENARIOS.toInt()
        iprotFormatCells = grbit and ALLOWFORMATCELLS == ALLOWFORMATCELLS.toInt()
        iprotFormatColumns = grbit and ALLOWFORMATCOLUMNS == ALLOWFORMATCOLUMNS.toInt()
        iprotFormatRows = grbit and ALLOWFORMATROWS == ALLOWFORMATROWS.toInt()
        iprotInsertColumns = grbit and ALLOWINSERTCOLUMNS == ALLOWINSERTCOLUMNS.toInt()
        iprotInsertRows = grbit and ALLOWINSERTROWS == ALLOWINSERTROWS.toInt()
        iprotInsertHyperlinks = grbit and ALLOWINSERTHYPERLINKS == ALLOWINSERTHYPERLINKS.toInt()
        iprotDeleteColumns = grbit and ALLOWDELETECOLUMNS == ALLOWDELETECOLUMNS.toInt()
        iprotDeleteRows = grbit and ALLOWDELETEROWS == ALLOWDELETEROWS.toInt()
        iprotSelLockedCells = grbit and ALLOWSELLOCKEDCELLS == ALLOWSELLOCKEDCELLS.toInt()
        iprotSort = grbit and ALLOWSORT == ALLOWSORT.toInt()
        iprotAutoFilter = grbit and ALLOWAUTOFILTER == ALLOWAUTOFILTER.toInt()
        iprotPivotTable = grbit and ALLOWPIVOTTABLES == ALLOWPIVOTTABLES.toInt()
        iprotSelUnlockedCells = grbit and ALLOWSELUNLOCKEDCELLS == ALLOWSELUNLOCKEDCELLS.toInt()
    }

    /**
     * update protection-style grbit variables + update record
     */
    private fun updateProtectionGrbit() {
        val b = ByteTools.cLongToLEBytes(grbit)
        this.getData()
        data[19] = b[0]
        data[20] = b[1]
        data[21] = b[2]
     ...
     * @param set
     */
    fun setProtectionOption(protectionOption: Int, set: Boolean) {
        if (set)
            grbit = grbit or protectionOption
        else
            grbit = grbit and protectionOption.inv()
     ...
     * @return
     */
    fun getProtectionOption(protectionOption: Int): Boolean {
        return grbit and protectionOption == protectionOption
    }
    ...
    companion object {
        ...
        val ALLOWOBJECTS: Short = 0x1
        val ALLOWSCENARIOS: Short = 0x2
        val ALLOWFORMATCELLS: Short = 0x4
        val ALLOWFORMATCOLUMNS: Short = 0x8
        val ALLOWFORMATROWS: Short = 0x10
        val ALLOWINSERTCOLUMNS: Short = 0x20
        val ALLOWINSERTROWS: Short = 0x40
        val ALLOWINSERTHYPERLINKS: Short = 0x80
        val ALLOWDELETECOLUMNS: Short = 0x100
        val ALLOWDELETEROWS: Short = 0x200
        val ALLOWSELLOCKEDCELLS: Short = 0x400
        val ALLOWSORT: Short = 0x800
        val ALLOWAUTOFILTER: Short = 0x1000
        val ALLOWPIVOTTABLES: Short = 0x2000
        val ALLOWSELUNLOCKEDCELLS: Short = 0x4000
        ...
        val prototype: XLSRecord?
            get() {
                val f = FeatHeadr()
                f.opcode = XLSConstants.FEATHEADR
