...
public final class Bof extends UnencryptedXLSRecord{
    ...
    public void setOffset(int s){
        super.setOffset(s);
        if(worksheet!=null)
            if(isSheetBof()||isVBModuleBof()||(worksheet.getMyBof()==this))worksheet.setLbPlyPos(this.getLbPlyPos());
    }
    ...
    */
    ...
    public boolean isSheetBof() {
        if((grbit & 0x10)==0x10)return true;
        return false;
    }
    
    public boolean isVBModuleBof() {
        if((grbit & 0x06)==0x06)return true;
        return false;
    ...
    public boolean isChartBof() {
        if((grbit & 0x20)==0x20)return true;
        return false;
    }
