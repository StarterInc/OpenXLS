...
*/
class Style : io.starter.formats.XLS.XLSRecord() {
    internal var ixfe: Short = 0
    internal var istyBuiltIn: Byte = 0
    internal var iLevel: Byte = 0
    internal var cch: Short = 0
    internal var rgch: String

    internal var builtIn = false
    ...
    override fun init() {
        super.init()
        val unstripped = ByteTools.readShort(this.getByteAt(0).toInt(), this.getByteAt(1).toInt())
        ixfe = (unstripped and 0x7FF).toShort()
        if (this.getByteAt(1) and 0x128 == 0x128) {
            builtIn = true
            istyBuiltIn = this.getByteAt(2)
            iLevel = this.getByteAt(3)
        } else {
            cch = this.getByteAt(2).toShort()
            if (cch < 0) cch *= -1
            val tmp = this.getBytesAt(3, cch.toInt())
            rgch = String(tmp!!)
        }
    ...
    companion object {
         ...
         * serialVersionUID
         */
        private val serialVersionUID = 8634444452254051296L
    }
