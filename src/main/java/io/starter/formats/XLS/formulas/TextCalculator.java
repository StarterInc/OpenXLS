...
public class TextCalculator {
    ...
    protected static Ptg calcAsc(Ptg[] operands) {
        ...
        io.starter.formats.XLS.WorkBook bk = operands[0].getParentRec().getWorkBook();
    ...
    protected static Ptg calcChar(Ptg[] operands) {
        ...
        if (s.intValue() > 255 || s.intValue() < 1) return PtgCalculator.getError();
        byte[] b = new byte[1];
        b[0] = s.byteValue();
        String str = "";
        try {
    ...
    protected static Ptg calcExact(Ptg[] operands) {
        ...
        if (s1.equals(s2)) return new PtgBool(true);
    ...
    protected static Ptg calcFind(Ptg[] operands) {
        ...
        if (o == null || oo == null) return new PtgErr(PtgErr.ERROR_VALUE);
...
*/
    ...
    protected static Ptg calcMid(Ptg[] operands) {
        String s = String.valueOf(operands[0].getValue());
        if (s == null || s.equals("")) return new PtgStr("");    //  Don't error out if "" ala Excel
        if (operands[1] instanceof PtgErr || operands[2] instanceof PtgErr) return new PtgErr(PtgErr.ERROR_VALUE);
        int start = operands[1].getIntVal() - 1;
        int len = operands[2].getIntVal();
        if (len < 0) {
            len = start + len;
        }
        if (s.length() < start) return new PtgStr("");
        if (start == -1) return new PtgErr(PtgErr.ERROR_VALUE);
        s = s.substring(start);
        if (len > s.length()) return new PtgStr(s);
        s = s.substring(0, len);
        return new PtgStr(s);
    }
    /*
  ...
  */
    ...
    protected static Ptg calcRight(Ptg[] operands) {
        if (operands.length < 1) return new PtgErr(PtgErr.ERROR_VALUE);
        String origstr = String.valueOf(operands[0].getValue());
        if (origstr.equals("")) return new PtgStr("");
        int numchars = operands[1].getIntVal();
        if (numchars > origstr.length()) numchars = origstr.length();
        if (numchars < 0) return new PtgErr(PtgErr.ERROR_VALUE);
    ...
    protected static Ptg calcSearch(Ptg[] operands) {
        ...
        if (i == -1) return new PtgErr(PtgErr.ERROR_VALUE);
...
//	            Logger.logWarn("getting formatted string value for :" + res.toString() + " failed: " + e.toString()) ;
        ...
        try {
            ...
            } else fmtx = new DecimalFormat(fmt);
    ...
    protected static Ptg calcTrim(Ptg[] operands) {
        ...
        if (res.length() > 0) {
            ...
            while (res.indexOf("  ") != -1) {
                ...
                String poststring = res.substring(i + 1);
    ...
    protected static Ptg calcValue(Ptg[] operands) {
        try {
            String s = String.valueOf(operands[0].getValue());
            if (s.equals("")) s = "0"; // Excel returns a zero for a blank value if VALUE is called upon it.
    ...
    private static byte[] getUnicodeBytesFromOp(Ptg op) {
        ...
        if (op instanceof PtgRef) {
            io.starter.formats.XLS.BiffRec rec = ((PtgRef) op).getRefCells()[0];
            if (rec instanceof io.starter.formats.XLS.Labelsst)
                strbytes = ((io.starter.formats.XLS.Labelsst) rec).getUnsharedString().readStr();
