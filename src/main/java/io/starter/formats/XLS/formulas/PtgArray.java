...
*/		
		if (record.length>8) { // means that array data has already been appeneded to end of record array; store in rgvals
    ...
    public void parseArrayComponents() {
    	...
    	for (int i=3;i<rgval.length;){	// 20090820 KSC: post array contains nc & nr so i should be initially 3 instead of 0 [BugTracker 2683]
    		if (rgval[i]==0) { // empty value
			   ...
			   }catch(UnsupportedEncodingException e){Logger.logInfo("decoding formula string in array failed: " + e);}
    			arrVals.add(strVal);
    			i += strLen;
    ...
    public void setVal(String arrStr){
    	...
    	if (arrStr.indexOf("{") != -1){ // SHOULDN'T -- see FormulaParser.getPtgsFromFormulaString
		...
		if (cols[0].length>=255) {	// 20090824 KSC:  apparently sometimes an intermediary calculations step can include > 256 array elements ...
			isIntermediary= true;
			nc= cols[0].length-1;
		}	
		...
		System.arraycopy(ByteTools.shortToLEBytes((short)(rows.length-1)), 0, databytes, 9, 2);		// nr-1  // 20090819 KSC: placed in wrong pt of record:  was 2,3 [BugTracker 2683]
    ...
    private byte[] valuesIntoByteArray(String constVal){
		...
		try{	// number?
			...
			try {
				...
				} else if (constVal==null || constVal.equals("")) {	// emtpy or null value
