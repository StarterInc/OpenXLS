...
// 	if(bitAttrSemi      > 0)   return "SEMI(";
        ...
        if(bitAttrGoto      > 0)   return ""; // this may be wrong, but as far as I can tell it is just internal for calc purposes.
        return "UNKNOWN(";
...
// Old version?		if(bitAttrSemi   > 0)return true; // it just shows that this is a volatile function
		if(bitAttrVolatile > 0)return true; // it just shows that this is a volatile function
...
//        if ((grbit & 0x1)== 0x1){bitAttrSemi = 1;}        
        if ((grbit & 0x1)== 0x1){bitAttrVolatile = 1;}  // volatile= a function that needs to be recalculated always, such as NOW()
        if ((grbit & 0x2)== 0x2){bitAttrIf = 1;}
        if ((grbit & 0x4)== 0x4){bitAttrChoose = 1;}
        if ((grbit & 0x8)== 0x8){bitAttrGoto = 1;}
        if ((grbit & 0x10)== 0x10){bitAttrSum = 1;}
        if ((grbit & 0x20)== 0x20){bitAttrAssign = 1;}	// changed name from bitAttrBaxcel
        if ((grbit & 0x40)== 0x40){bitAttrSpace = 1;}
        if ((grbit & 0x41)== 0x41){bitAttrSpaceVolatile = 1;}
