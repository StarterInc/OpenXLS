...
//                    total += Double.parseDouble(String.valueOf(ov));
	...
	*/
	protected static Ptg calcAveDev(Ptg[] operands){
		...
		if (average == -0.001)return PtgCalculator.getError();
...
*/
	protected static Ptg calcCountBlank(Ptg[] operands){
	 ...
	 BRUTAL!!
	 */
	 protected static Ptg calcCountif(Ptg[] operands)
	 throws FunctionNotSupportedException{
	 	if (operands.length != 2)return PtgCalculator.getError();
	 				...
	 				if (o != null){
	 					...
	 					if (donumber){
	 						try{
	 							...
	 							if (matchDub == matchDub2) count++;
	 						}catch(NumberFormatException e){}; 
	 					} else{
	 						if (matchStr.equalsIgnoreCase(match2)) count++;		
	 					}				
	 				}
	 		   }
	 		}else{  // it's a single ptgref
 ...
 */
	 ...
	 */
	 protected static Ptg calcIntercept(Ptg[] operands) throws CalculationException{
	    ...
	    if (xvals==null || yvals==null) return new PtgErr(PtgErr.ERROR_NA);//20090130 KSC: propagate error
		double sumXVals = 0;
		for (int i=0;i<xvals.length;i++){
			sumXVals += xvals[i];
		}
 ...
 */
 protected static Ptg calcLineSt(Ptg[] operands) throws CalculationException{
    ...
    if (ys==null) return new PtgErr(PtgErr.ERROR_NA);//20090130 KSC: propagate error
 	double[] xs;
 	if (operands.length==1 || operands[1] instanceof PtgMissArg) {
		...
		if (xs==null) return new PtgErr(PtgErr.ERROR_NA);//20090130 KSC: propagate error
	}
	...
	if (ps instanceof PtgErr) return ps;
 ...
 protected static Ptg calcMax(Ptg[] operands){
     ...
     for (int i=0;i<operands.length;i++){
         ...
         if (pthings != null){
             ...
             try{  // some fields may be text, so handle gracefully
                 if (resPtg.getValue() != null) d = new Double(String.valueOf(resPtg.getValue()));
                 if (d.doubleValue() > result){result = d.doubleValue();} 
             }catch(NumberFormatException e){};
         }else{
             Ptg p = operands[i];
             try{
 ...
 protected static Ptg calcMedian(Ptg[] operands){
	 if (operands.length<1) return new PtgErr(PtgErr.ERROR_VALUE);
	  ...
	  try {
		  ...
		  return pnum;
	  } catch (ArrayIndexOutOfBoundsException e) {	// 20090701 KSC: catch exception
		  return new PtgErr(PtgErr.ERROR_VALUE);
	  }
  }
 /**
...
protected static Ptg calcMin(Ptg[] operands){
 ...
 for (int i=0;i<operands.length;i++){
     ...
     if (pthings != null){
         ...
         try{  // some fields may be text, so handle gracefully
         	if (resPtg instanceof PtgErr) return resPtg;	// 20090205 KSC: propagate error
             if (resPtg.getValue() != null) { 
             	d = new Double(String.valueOf(resPtg.getValue()));
                 // 20090129 KSC; if (d.doubleValue() < result || result == -1){result = d.doubleValue();} // 20070215 KSC: only access d if not null!
                 if (d.doubleValue() < result){result = d.doubleValue();} // 20070215 KSC: only access d if not null!
             }
         }catch(NumberFormatException e){}
         catch(NullPointerException e) { }// 20070209 KSC
     }else{
         Ptg p = operands[i];
         try{
         	Object ov = p.getValue();
 ...
 */
 protected static Ptg calcNormdist(Ptg[] operands){
	 if (operands.length<4) return new PtgErr(PtgErr.ERROR_VALUE);
	 try {
		 ...
		 if (stddev <= 0) return new PtgErr(PtgErr.ERROR_NUM);
...
Public Function NormSInv(ByVal p As Double) As Double
	...
	private static double quartile(double p, double mu, double sigma)
		{
		    ...
		    double p_, q, r, val;
		    if (p==R_DT_0)     return Double.NEGATIVE_INFINITY;
		    if (p==R_DT_1)     return Double.POSITIVE_INFINITY;
		    //R_Q_P01_check(p);
 	...
 	public static Ptg calcNormInv(Ptg[] operands) {
 		try {
 	 		...
 	 		if (p < 0 || p > 1) return new PtgErr(PtgErr.ERROR_NUM);
 	 		double mean= operands[1].getDoubleVal();
 	 		double stddev= operands[2].getDoubleVal();
 	 		if (stddev <= 0) return new PtgErr(PtgErr.ERROR_NUM);
 ...
 {	 
	 if (operands.length!=1) return PtgCalculator.getValueError();
	 try {
		 double x= operands[0].getDoubleVal();
		 if (x < 0 || x > 1)
		        return new PtgErr(PtgErr.ERROR_NUM);
...
/* below is not accurate enough - but N-R approximation is impossible ((:*/ 		 
	     ...
	     if ( x < plow ) {
	              ...
	              BigDecimal r= new BigDecimal((((((c[0]*q+c[1])*q+c[2])*q+c[3])*q+c[4])*q+c[5]) /
	     ...
	     double q = x - 0.5;
	     double r = q*q;
	     BigDecimal rr= new BigDecimal((((((a[0]*r+a[1])*r+a[2])*r+a[3])*r+a[4])*r+a[5])*q /
 ...
 */
 protected static Ptg calcStdev(Ptg[] operands) throws CalculationException{
 	  ...
 	  if (allVals==null) return new PtgErr(PtgErr.ERROR_NA);//20090130 KSC: propagate error
	  double sqrDev = 0;
	  for (int i = 0;i<allVals.length;i++){
 ...
 */
 public static Ptg calcSteyx(Ptg[] operands) throws CalculationException{
    ...
    if (y==null) return new PtgErr(PtgErr.ERROR_NA);//20090130 KSC: propagate error
	yVarp *= y.length;
	xVarp *= y.length;
...
// KSC: THIS FUNCTION DOES NOT WORK AS EXPECTED: TODO: FIX!
 ...
 if (true) return new PtgErr(PtgErr.ERROR_VALUE);	 
