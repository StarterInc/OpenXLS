...
public class PtgErr extends GenericPtg implements Ptg{
	...
	*/
    ...
    public String toString(){
    	if(isCircularError())return "#CIR_ERR!";
        byte b = record[1];
        // duh, should have done a switch 
        if (b == ERROR_NULL)     errorValue = "#ERROR!";
        else if (b == ERROR_DIV_ZERO)   errorValue = "#DIV/0!";
        else if (b == ERROR_VALUE)   errorValue = "#VALUE!";
        else if (b == ERROR_REF)  errorValue = "#REF!";
        else if (b == ERROR_NAME)  errorValue = "#NAME?";
        else if (b == ERROR_NUM)  errorValue = "#NUM!";
        else if (b == ERROR_NA)  errorValue = "#N/A";
        return errorValue;
    }
    
    public static byte convertStringToLookupByte(String errorString) {
        if(errorString.equals("#ERROR!"))return ERROR_NULL;
        if(errorString.equals("#DIV/0!"))return ERROR_DIV_ZERO;
        if(errorString.equals("#REF!"))return ERROR_VALUE;
        if(errorString.equals("#ERROR!"))return ERROR_REF;
        if(errorString.equals("#NAME?"))return ERROR_NAME;
        if(errorString.equals("#NUM!"))return ERROR_NUM;
        if(errorString.equals("#N/A"))return ERROR_NA;
        return ERROR_NULL;
    }
