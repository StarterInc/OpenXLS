...
public class PtgRef3d extends PtgRef implements Ptg, IxtiListener{
    ...
    * 
    * @see io.starter.formats.XLS.formulas.Ptg#setLocation(java.lang.String)
    */
	...
	*/
	public void populateVals(){
        ...
        if ((column & 0x8000) == 0x8000){ // is the Row relative?
            fRwRel = true; 
        }else{ fRwRel = false;}
        if ((column & 0x4000) == 0x4000){ // is the Column relative?
            fColRel = true;
        }else{ fColRel = false;}
	...
	*/
     ...
     * @see io.starter.formats.XLS.formulas.PtgArea3d#setReferencedSheet(io.starter.formats.XLS.Boundsheet)
     */
   ...
   */
    ...
    public String getSheetName() {
    	if (this.sheetname==null) {
        	if (parent_rec!=null) {
    			WorkBook wb = parent_rec.getWorkBook();
    			if(wb!=null && wb.getExternSheet()!=null) {	// 20080306 KSC: new way is to get sheet names rather than sheets as can be external refs
	...
	public BiffRec[] getRefCells() {
	    if(sheetname==null)sheetname = this.getSheetName();
