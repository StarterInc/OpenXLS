...
private static Complex imParseComplexNumber(String complexNumber) 
	...
	if (complexNumber.length() > 0) {
		try {
			...
			if (complexNumber.substring(i-1, i).equals("i") || 
					complexNumber.substring(i-1, i).equals("j")) {
				c.suffix= complexNumber.substring(i-1, i);
				i-=2;
				while (i >= 0 && 
						!(complexNumber.substring(i, i+1).equals("+") ||
								complexNumber.substring(i, i+1).equals("-")))
						  i--;
 ...
 */
	protected static Ptg calcImAbs(Ptg[] operands) {
		...
		catch (Exception e) { return new PtgErr(PtgErr.ERROR_NUM); }
