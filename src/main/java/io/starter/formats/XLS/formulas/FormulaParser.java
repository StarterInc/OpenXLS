    ...
    public static void adjustParameterIds(Stack expression){
        ...
        while(!newstck.isEmpty()){
	        ...
	        if (p.getIsControl()) {
				...
				if (p.getOpcode() == 0x22 || p.getOpcode() == 0x42 || p.getOpcode() == 0x62){t=p.getNumParams();}// it's a ptgfuncvar
		   		if (p.getOpcode() == 0x21 || p.getOpcode() == 0x41 || p.getOpcode() == 0x61){t=p.getNumParams();}// it's a ptgFunc
