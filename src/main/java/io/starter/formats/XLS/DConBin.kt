 ...
 */
class DConBin : XLSRecord(), XLSConstants {
    private var nBuiltin: Byte = 0
    ...
    override fun init() {
        ...
        cchFile = ByteTools.readShort(this.getByteAt(5).toInt(), this.getByteAt(6).toInt())
        if (cchFile > 0)
    ...
    companion object {
        ...
        val prototype: XLSRecord?
            get() {
                val db = DConBin()
                db.opcode = XLSConstants.DCONBIN
                db.setData(byteArrayOf(0, 0, 0, 0, 0, 0))
                db.init()
                return db
            }
    }
