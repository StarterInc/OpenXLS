	...
	*/
	public WorkBook() {
		...
		if (System.getProperties()
				.get("io.starter.OpenXLS.sharedupes") != null) {
			this.sharedupes = System.getProperties()
					.get("io.starter.OpenXLS.sharedupes").equals("true");
			if (this.sharedupes) {
			...
			for (int k = 0; k < p.length; k++) {
				if (p[k] instanceof PtgRef) {
					...
					try {
						...
						Logger.logWarn("External Reference encountered upon updating formula references:  Worksheet Reference Found: "
								+ ptg.getSheetName());
						...
						for (int j = 0; j < pe.length; j++) {
							if (pe[j] instanceof PtgRef) {
								...
								try {
									...
									Logger.logWarn("External References Not Supported:  UpdateFormulaReferences: External Worksheet Reference Found: "
											+ ptg.getSheetName());
									ptg.setExternalReference(origWorkBookName);
								}
							}
