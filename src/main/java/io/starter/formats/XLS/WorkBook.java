...
public class WorkBook implements Serializable, XLSConstants, Book {
	...
	void initImages() 	 {	
		...
		for(int i=0;i<msodg.getMsodrawingrecs().size();i++){	// 20070914 KSC: store msodrawingrecs with MSODrawingGroup instead of here
    ...
    public void prestream(){
    	   if (this.getMSODrawingGroup()!=null)this.getMSODrawingGroup().prestream();
     }
    ...
    public Supbook getExternalSupbook(String externalWorkbook, boolean bCreate) {
        Supbook sb= null;
        if (externalWorkbook==null) return null;
...
//          this.addRecord(sb,false); // "" no need
    ...
    */
    ...
    public short getFormatId (String pattern) {
        ...
        if (res != null) return res.shortValue();
        return -1;
    }
    ...
    private void initBuiltinFormats() {
        ...
        for (int i = 0; i < formats.length; i++)
            formatlookup.put(
            		formats[i][0].toUpperCase(),
            		Short.valueOf( formats[i][1], 16 ) );
    }
    ...
    */
    ...
    public int getExtenalNameNumber(String s) {
        ...
        if (i > -1) // got it
            return i+1;
        if (this.getExternSheet() == null)this.addDefaultExternsheet();
        ...
        }catch(Exception e){Logger.logWarn("Error adding externname: " + e);}
        return n;
    }
    ...
    */
    ...
    public int getNameNumber(String nameStr) {
        for (int i = 0;i<names.size();i++){
            ...
            if (n.getName().equalsIgnoreCase(nameStr)) return i+1;          
        }
...
//        myName.setName(nameStr);
    ...
    public int getFontIdx(Font f){
    	...
    	for (int i= fonts.size()-1; i >=0; i--) {	// start from the back so don't initially match defaults...
    ...
    */
    public void setSelectedSheet(Boundsheet bs) {
        ...
        for(int t=0;t<bsx.length;t++) {
            if(bsx[t]!=bs)bsx[t].setSelected(false);
        }
    ...
    */
    public BiffRec addRecord(BiffRec rec, boolean addtorec) {
       ...
       if(bs!=null)
        ...
        if ((bs != null)) { // &&){ 
            lastbound = bs;	
            if(addtorec) rec.setSheet(bs);// we don't include Bof or other Book-recs because it lives in the Streamer recvec
            ...
            && */(!copying) {
              if((lastFormula != null) && (opc== STRINGREC)){
                    ...
                    if (currchart==null) bs.addCell((CellRec) rec);
        ...
        if(inChartSubstream) {
            if (currchart == null) {
                if(rec.getOpcode()==CHART) {
                    charts.add(rec);
                    if (bs!=null) bs.addChart((Chart)rec);
                    currchart = (Chart) rec;
        ...
        if (opc==XLSRecord.DBCELL 
                || opc==XLSRecord.ROW 
                || rec.isValueForCell() 
                || opc==XLSRecord.MULRK 
                /*|| opc==MULBLANK*/ 
                || opc==CHART 
                || opc==XLSRecord.FILEPASS
                || opc==XLSRecord.SHRFMLA
        		|| opc==XLSRecord.ARRAY 
        ...
        if(addtorec) {
            ...
            switch (opc) {
                ...
                case FEATHEADR:
                	...
                	if (bs != null) manager = bs.getProtectionManager();
                	else manager = this.getProtectionManager();                	
                	manager.addRecord( rec );
                    break;
                    
                case DVAL:
                        if(bs!=null)bs.setDvalRec((Dval) rec);
                    break;
                ...
                case INDEX :
                    ...
                    if (bs == null) {
                        Logger.logWarn(
                            "ERROR: WorkBook.addRecord( Index ) error: BAD LBPLYPOS.  The wrong LB:" + lbplypos);
                        try {
...
/*                case SHRFMLA : done in shrfmla.init
                ...
                case DSF:
                    ...
                    if(dsf.fDSF == 1){
                        ...
                        throw new WorkBookException("ERROR: DOUBLE STREAM FILE DETECTED!  OpenXLS is compatible with Excel 97 + only.", WorkBookException.DOUBLE_STREAM_FILE);
                    }
                    break;
                ...
                case BOF :
                    if(DEBUGLEVEL > 5)Logger.logInfo("BOF:" + bofct + " - " + rec);
                    if (eofct == bofct) {
                        if (bs != null) {
                            bs.setBOF((Bof) rec);
                        }
                ...
                case SUPBOOK:   // KSC: must store ordinal positions of SupBooks, for adding Externsheets
                    supBooks.add((Supbook) rec);
                    if (myADDINSUPBOOK==null) { // see if this is the ADD-IN SUPBOOK rec
                ...
                case BOUNDSHEET :
                    ...
                    */
                    ...
                    if (win1!=null) {// Windows1 record is optional 20101004 TestCorruption.TestNPEOnOpen 
			...
			case MSODRAWING :
				...
				if (msodg!=null)				
					...
					// do what???io.starter.OpenXLS.util.Logger.log("PROBLEM with MSODG!");
				break;
    ...
    */
    public BiffRec getCell(String cellname)
    ...
    */
    public BiffRec getCell(String sheetname, String cellname)
        throws CellNotFoundException, WorkSheetNotFoundException {
        cellname = cellname.toUpperCase();
        try {
            Boundsheet bs = (Boundsheet) this.getWorkSheetByName(sheetname);
    ...
    */
    public void removeWorkSheet(Boundsheet sheet) {
        ...
        if(this.getNumWorkSheets()==0)return; // empty book
    ...
    */
    public Boundsheet getWorkSheetByName(String bstr)
...
*/
public Boundsheet getWorkSheetByHash(String s)
    throws WorkSheetNotFoundException {
    ...
    */
    public Boundsheet getWorkSheetByNumber(int i)
        throws WorkSheetNotFoundException {
            Boundsheet bs = null;   
            try{
    ...
    */
    public Xf getXf(int i) {
        if(xfrecs.size()<(i-1))return null;
    ...
    public void updateFormatCache(Xf xf) {
    	if (xf.tableidx!=-1) {	// if this xf has been already added to the workbook 
			if (formatCache.containsValue(xf)) {	// xf signature has changed/it's been updated
     ...
     * not copied
    */
    public void copyWorkSheet(String SourceSheetName, String NewSheetName)
        throws Exception {
    ...
    */
     ...
     */
    public void deleteChart(String chartname, Boundsheet sheet) 
    ...
    */
    public void addBoundsheet(byte[] inbytes, String origSheetName, String NewSheetName, String origWorkBookName, boolean SSTPopulatedBoundsheet) throws IOException, ClassNotFoundException {
    ...
    */
    public void addBoundsheet(Boundsheet bound, String origSheetName, String newSheetName, String origWorkBookName, boolean SSTPopulatedBoundsheet) {
        ...
        } catch (WorkSheetNotFoundException we) { ; /* good !!!*/ }
        ...
        XLSRecord x = null;
        if(lastbound!=null){
        	try{	// lastbound must be reset because other operations could alter
        ...
        if(this.myexternsheet == null) this.addExternsheet();
        try { 
        ...
        for (int i= 0; i < chts.size(); i++) {
            Chart chart = (Chart) chts.get(i);	// obviously algorithm has changed and chart is NOT removed :) [discovered by Shigeo/Infoteria/formatbroken273193.sce] // 20080702 KSC: since it's removed, don't inc index            
        ...
        for (int z = 1; z < numrecs; z++) {
            ...
            if (DEBUGLEVEL > 5)try{Logger.logInfo("Copying: "+ xl.toString()+ ":"+ String.valueOf(newoffset)+ ":"+ xl.getLength());}catch(Exception e){;}
            if (xl instanceof Codename){
            ...
            } else if (xl instanceof Cf) {	// must check Conditional Format formula refs and handle any external references 
            	try {
            ...
            }else if (xl.getOpcode()==MSODRAWING || (xl.getOpcode()==CONTINUE && ((Continue)xl).maskedMso!=null)) {	// 20100510 KSC: handle masked mso's            	
            	MSODrawing mso;						
        		...
        		if (mso.getImageIndex()>0) { //  add image bytes as well, if any        			
        			ImageHandle im= bound.getImageByMsoIndex(mso.getImageIndex());
        			int idx= msodg.addImage(im.getImageBytes(), im.getImageType(), false); 
        			bound.imageMap.put(im, Integer.valueOf(idx));	// 20100518 KSC: makes more sense? im.getImageIndex()));	// add new image to map and link to actual imageIndex - moved from above
        ...
        if (msodg!=null) {// Moved from above so don't udpate at every mso addition
        ...
        if (DEBUGLEVEL > 5)Logger.logInfo("changesize for  new boundsheet: "    + bound.getSheetName()+ ": "    + tout);
        copying = false;
    }
    ...
    private void updateTransferedCellReferences(Boundsheet bound, String origSheetName, String origWorkBookName) {        
        ...
        List boundFonts = (List)bound.getTransferFonts();		// ALL fonts in the source workbook
        ...
        for (int i=0;i<rows.length;i++) {
            ...
            if (rows[i].getIxfe()!=this.getDefaultIxfe())
            	transferFormatRecs(rows[i], localFonts, boundFonts, localXfs, boundXfs);	// 20080709 KSC: handle default ixfe for row
            ...
            while (rowcells.hasNext()) {
                ...
                if (b instanceof Formula) { // Examine Ptg Refs to handle external sheet references not contained in this workbook
				...
				if (p[k] instanceof PtgRef) {
					...
					try {								
    					if (!(ptg instanceof PtgArea3d) || 
    							((PtgArea3d) ptg).getFirstSheet().equals(((PtgArea3d) ptg).getLastSheet())){ 
...
// don't understand this			this.getWorkSheetByName(pref.getLastPtg().getSheetName());
						...
						Logger.logWarn("External Reference encountered upon updating formula references:  Worksheet Reference Found: " + ptg.getSheetName());
					...
					for (int j= 0; j < pe.length; j++) {
						if (pe[j] instanceof PtgRef) {
        					...
        					try {
            					if (ptg instanceof PtgArea3d)  { // PtgRef3d, etc.
            						this.getWorkSheetByName(ptg.getSheetName());
                					ptg.setLocation(ptg.toString());	// reset ixti if nec.
            					}
            					// otherwise, we're good
        					} catch (WorkSheetNotFoundException we) {
        						Logger.logWarn("External References Not Supported:  UpdateFormulaReferences: External Worksheet Reference Found: " + ptg.getSheetName());
        						ptg.setExternalReference(origWorkBookName);
        					}
						}
					}
	 ...
	 */
    private void transferFormatRecs(BiffRec b, HashMap<String, Integer> localFonts, List boundFonts, HashMap<String, Integer> localXfs, List boundXfs) {
	 ...
	 * @param boundXfs		List of string version of all xfs, xf nums in external workbook
	 */
    private int transferFormatRecs(int oldXfNum, HashMap<String, Integer> localFonts, List boundFonts, HashMap<String, Integer> localXfs, List boundXfs) {
    	int localNum = -1;
    	if (boundXfs.size()>oldXfNum) {// if haven't populatedForTransfer i.e. haven't opted to transfer formats ...
    ...
    throws ChartNotFoundException{
        ...
        for(int x=0;x<cv.size();x++){
            ...
            if(titlemso!=null){
            	...
            	if(mson.equalsIgnoreCase(chartname))return cht;
            }
        }
        ...
        for(int x=0;x<cv.size();x++){
            ...
            if(cname.equalsIgnoreCase(chartname))return cht;
...
/*    	List records= this.getStreamer().records;
    ...
    public void addExternsheet(){
        if (myexternsheet == null){
            ...
            try{
                ...
                this.addRecord(ex,false);
                myexternsheet = ex;
            }catch(WorkSheetNotFoundException e){Logger.logWarn("WorkBook.addExternSheet() locating Sheet for adding Externsheet failed: " + e);}
	 ...
	 * @param b
	 */
	public void setIsExcel2007(boolean b) { isExcel2007= b; }
	/**
	 * returns truth of "Excel 2007" format
	 */
	public boolean getIsExcel2007() { return isExcel2007; }
	/**
	 *	returns the workbook codename used by vba macros OOXML-specific
	 */
	public String getCodename() { return ooxmlcodename; }
    ...
    * @return boolean
	 ...
	 * @param s
	 */
	public void setCodename(String s) { ooxmlcodename= s; }	// TODO: input into Codename record
	 ...
	 */
	public void setDxfs(ArrayList dxfs) { this.dxfs= dxfs; }
	/**
	 * returns the list of dxf's (incremental style info) (int) OOXML-specific
	 */
	public ArrayList getDxfs() { return dxfs; }
     ...
     */
    public void setPivotCache(PivotCache pc) { ptcache= pc; }

    /**
     * return the pivot cache, if any
     * @return
     */
    public PivotCache getPivotCache() { return ptcache; }
    /**
    ...
    public void closeSheets() {
    	for (int i= boundsheets.size()-1; i > 0; i--) {
    	    ...
    	    if (b.streamer!=null) {	// do separately because may call boundsheet close
    	    	b.streamer.close();
    	    	b.streamer= null;
    	    }
    		b.close();
