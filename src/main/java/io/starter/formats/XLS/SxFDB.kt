...
class SxFDB : XLSRecord(), XLSConstants, PivotCacheRecord {
    private var ifdbParent: Short = 0
    private var ifdbBase: Short = 0
    private var csxoper: Short = 0
    private var cisxoper: Short = 0
    private var catm: Short = 0
    private var grbit: Short = 0
    /**
     * returns the cache field name == cache row cell label
     */
    var cachefield: String? = null
        private set
    // significant bit fields
    internal var fAllAtoms: Boolean = false
    internal var fRangeGroup: Boolean = false
    internal var fNumField: Boolean = false
    internal var fTextEtcField: Boolean = false
    internal var fnumMinMaxValid: Boolean = false
    internal var fNonDates: Boolean = false
    internal var fDateInField: Boolean = false
    internal var fCalculatedField: Boolean = false
    internal var fShortItms: Boolean = false
    ...
    var nCacheItems: Int
        ...
        set(n) {
            catm = n.toShort()
            var b = ByteTools.shortToLEBytes(catm)
            this.getData()[12] = b[0]
            this.getData()[13] = b[1]
            ...
            if (n > 0) {
                ...
                fAllAtoms = false
                grbit = grbit and 0x1.inv().toShort()
            }
    ...
    override fun init() {
        ...
        // grouping-related
        ifdbParent = ByteTools.readShort(this.getByteAt(2).toInt(), this.getByteAt(3).toInt())    // specifies the cache field index, as specified by Cache Fields, of the grouping cache field for this cache field
        ifdbBase = ByteTools.readShort(this.getByteAt(4).toInt(), this.getByteAt(5).toInt())        // specifies the cache field index, as specified by Cache Fields, of the base cache field
        //ignored:  citmUnq= ByteTools.readShort(this.getByteAt(6),this.getByteAt(7)); // "Undefined and MUST be ignored."
        csxoper = ByteTools.readShort(this.getByteAt(8).toInt(), this.getByteAt(9).toInt())        // specifies the number of cache items in this cache field when this cache field is a grouping cache field,
        cisxoper = ByteTools.readShort(this.getByteAt(10).toInt(), this.getByteAt(11).toInt())    // specifies the number of cache items in the base cache field that are grouped by this cache field
        // + fHasParent, fRangeGroup ...
        catm = ByteTools.readShort(this.getByteAt(12).toInt(), this.getByteAt(13).toInt())        // number of cache items
        val cch = ByteTools.readShort(this.getByteAt(14).toInt(), this.getByteAt(15).toInt()).toInt()
        if (cch > 0) {        // 0xFFFF if none
            val encoding = this.getByteAt(16)
            val tmp = this.getBytesAt(17, cch * (encoding + 1))
            try {
                if (encoding.toInt() == 0)
                    cachefield = String(tmp!!, XLSConstants.DEFAULTENCODING)
                else
                    cachefield = String(tmp!!, XLSConstants.UNICODEENCODING)
            } catch (e: UnsupportedEncodingException) {
                Logger.logInfo("SXString.init: $e")
            }

        }
        fAllAtoms = grbit and 0x1 == 0x1
        // KSC: TESTING
        //if (!fAllAtoms)
        //	Logger.logWarn("SXFDB: not all cache items are used");
        fRangeGroup = grbit and 0x10 == 0x10            // A bit that specifies whether this cache field is grouped by using numeric grouping or date grouping, as specified by Grouping.
        fNumField = grbit and 0x20 == 0x20            // at least one numeric field (SxNum records follow)
        fTextEtcField = grbit and 0x80 == 0x80        // text field (SxString records follow)... if fNumField must be false
        fnumMinMaxValid = grbit and 0x100 == 0x100    // true date or num field
        fShortItms = grbit and 0x200 == 0x200            // true if > 255 cache items
        fNonDates = grbit and 0x400 == 0x400
        fDateInField = grbit and 0x4000 == 0x4000        // at least one date field (SxDtr follows)
        fCalculatedField = grbit and 0x8000 == 0x8000    // Sxformulas follow "A calculated field is a cache field (section 2.2.5.3.5) and does not correspond to a column in the source data (section 2.2.5.3.2). The values for a calculated field are calculated based on the formula specified for the calculated field"
        if (DEBUGLEVEL > 3)
            Logger.logInfo(this.toString())
    }

    override fun toString(): String {
        return String.format("SXFDB -p: %d d: %d csxoper: %d cisxoper: %d catm: %d stFieldName: %s fAllAtoms? %b text? %b num? %b calculated? %b", ifdbParent, ifdbBase, csxoper, cisxoper, catm, cachefield, fAllAtoms, fTextEtcField, fNumField, fCalculatedField)
    }
     ...
     * @param s
     */
    fun setCacheField(s: String) {
        cachefield = s
        var strbytes = ByteArray(0)
        if (cachefield != null) {
            try {
                strbytes = cachefield!!.toByteArray(charset(XLSConstants.DEFAULTENCODING))
            } catch (e: UnsupportedEncodingException) {
        ...
        val newdata = ByteArray(cch + 3)    // account for encoding bytes + cch
        System.arraycopy(nm, 0, newdata, 0, 2)
        System.arraycopy(strbytes, 0, newdata, 3, cch.toInt())
     ...
     * @param type
     */
    fun setCacheItemsType(type: Int) {
        when (type) {
            ...
            XLSConstants.TYPE_FORMULA -> fCalculatedField = true
            //TYPE_BOOLEAN = 4,
            6// date
            -> {
                fTextEtcField = false
        ...
        if (fTextEtcField)
            ...
            grbit = grbit and 0x80.inv().toShort()
        if (fnumMinMaxValid)
            grbit = grbit or 0x100
        else
            grbit = grbit and 0x100.inv().toShort()
        if (fNonDates)
            grbit = grbit or 0x400
        else
            grbit = grbit and 0x400.inv().toShort()
        if (fDateInField)
            grbit = grbit or 0x4000
        else
            grbit = grbit and 0x4000.inv().toShort()
        if (fCalculatedField)
            grbit = grbit or 0x8000
        else
            grbit = grbit and 0x8000.inv().toShort()
    ...
    companion object {
        ...
        val prototype: XLSRecord?
            get() {
                val sxfdb = SxFDB()
                sxfdb.opcode = XLSConstants.SXFDB
