...
//				Logger.logInfo("Initializing Directory: "	+ name + ".  Start=" + rec.getStartBlock() + ". Size=" + recsize);
		...
		if (LEOFile.DEBUG)
			Logger.logInfo(
				"Total Size used by Directories : " + String.valueOf(totrecsize));
		...
		if (false && LEOFile.DEBUG) {
			Logger.logInfo(
				/*"INFO: StorageTable.addStorage() Storage size: "
					+*/ rec.getName()
					+ " Size: "
					+ String.valueOf(rec.getActualFileSize()) 
					+ " Start Block: "
					+ rec.getStartBlock());
		}
	}
		...
		for (int i= 0; i < directoryVector.size(); i++) {
			...
			if (n.equals("Root Entry")) {
				...
				*/	
				...
				BiffRec rec = new io.starter.formats.XLS.XLSRecord();		// 4 bytes are header ...		
