...
public class Storage extends BlockByteReader {
    ...
    public BlockByteReader getBlockReader() {
        BlockByteReader ret =
                new BlockByteReader(myblocks, this.getActualFileSize());
        return ret;
    }
    ...
    public void initFromMiniStream(List miniStream, int[] miniFAT) throws LEOIndexingException {
        ...
        if (LEOFile.DEBUG) {
            if ((int) Math.ceil(this.getActualFileSize() / 64.0) != myblocks.size())
                Logger.logErr("Number of miniStream Sectors does not equal storage size.  Expected: " + (int) Math.ceil(this.getActualFileSize() / 64.0) + ". Is: " + myblocks.size());
        }
        ...
        for (int i = startBlock; i < FAT.length; ) {
            ...
            switch (nextIdx) {
                ...
                default: // normal block
                    ...
                    } else if (!thisbb.getIsSpecialBlock()) { // just skip as probably a bbdix in the midst of the secid chain
                        this.addBlock(thisbb); //
    ...
    public void addBlock(Block b) {
        ...
        if (b.getInitialized()) {
            if (LEOFile.DEBUG) Logger.logWarn("ERROR: " + this.toString() + " - Block is already initialized.");
            return;
        }
        b.setStorage(this);
