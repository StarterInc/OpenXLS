...
public class LEOFile implements Serializable{
	...
	*/
    ...
    public LEOHeader getHeader() { return header; }
...
//	    FAT= null;
    ...
    public void shutdown() {
        try{
            ...
            if(DEBUGLEVEL > 0)Logger.logWarn("could not close workbook cleanly." + e);
        }
    }
	...
	*/
	...
	private void initFromPrototype(String fname) {
	    try{
            ...
            if (b == null) {throw new io.starter.formats.XLS.WorkBookException(
    ...
    */
    ...
    public static boolean checkIsLEO (byte[] data, int count) {
        if (count < LEOHeader.majick.length) return false;
        for (int idx = 0; idx < LEOHeader.majick.length; idx++)
            if (data[ idx ] != LEOHeader.majick[ idx ]) return false;
        return true;
    }
	...
	*/
	...
	RootStorage rootStore = null;
		...
		while (e.hasMoreElements()) {
			...
			if (thisStore!=book && thisStore!=rootStore) { // && thisStore!=encryptionInfo?
				storages.add(thisStore);
				if (thisStore.getBlockType() == Block.SMALL) {	// count number of miniStream blocks
		...
		if (numMiniFATSectors > 0) {
			Storage[] sbs= buildMiniFAT(storages, numMiniFATSectors);	// returns two non-directory storages:  miniStream Storage + miniFAT index 
			storages.add(sbs[0]);
		...
		if(isEncrypted) {  // Encrypted workbooks are already set to correct size
		    numblocks--;
		...
		for (int i= 0; i < FAT.length; i++)	FAT[i]= -1;  // create byte array of -1's 128 bytes - total number of sector ids 
		...
		if(!isEncrypted) {
		    ...
		    if (DEBUG) Logger.logInfo("Workbook actual bytes: " + book.getActualFileSize());
		}
		
		// now rest of "static" stores (summary, document summary, comp obj ...)
		for (int t=0; t < storages.size(); t++) {
			Storage nstr = (Storage) storages.get(t);
			if (!nstr.miniStreamStorage) {	// miniStream blocks are handled separately
				if (nstr.getName().equals("miniStream")) {	// miniStream (short sector container)
 				...
 				} else if (nstr.getName().equals("miniFAT")) { // miniFAT (short sector) index	
					sbidxpos= blockpos;		
				}
...
//				nstr.setActualFileSize(0);	// default, 0 bytes
				...
				if (blks!=null) {
					...
					for (int i= 0; i < blks.length-1; i++) {	// -1 to trap last block with special end-of-block flag (-2) 
...
//					nstr.setActualFileSize((blks.length)*512);	// set file size -- see above why we can't do this at this time
		...
		if (numFATSectors > MAXDIFATLEN) {	// handle extra blocks - need extra blocks to store the indexes
...
//			handle Extra blocks beyond the 109*128 blocks described by the regular DIFAT (109 limit)
	...
	private Storage[] buildMiniFAT(List storages, int numsbs) {
		...
		for (int i= 0; i < sbidx.length; i++)	sbidx[i]= -1;  // init sbidx index
		int z= 0;
...
// 20100407 KSC: this screws up subsequent usages of book after writing ... see TestCreateNewName NPE 
		...
		Block[] smallBlocksToBig=
				BlockFactory.getBlocksFromByteArray(smallblocks, Block.BIG);
	 ...
	 final static void initSmallBlockIndex(int[] newidx, Block b) {
		while (b.hasNext()&& !(b==(Block)b.next())) {
			...
			if (origps < 0) {
				...
				if (false)Logger.logInfo("INFO: LEOFile Initializing block index: "+ origps+ " val: "+ newp+ " idxlen: "+ newidx.length);
				newidx[origps] = newp;
			}
			b = (Block) b.next();
		}
...
*/
    ...
    */
     ...
     * @return
     */
    public StorageTable getDirectoryArray() { return directories; }
...
//			StorageTable.writeitout(FATSectors, "FAT.dat");
	...
	public  final static FileBuffer readFile(File fpath) {
	    ...
	    String tmpfu = (String)System.getProperties().get("io.starter.formats.LEO.usetempfile");
	...
	*/
	private final static int[] readFAT(byte[] data) {
		...
		for (int i = 0; i < data.length;) {
			bbs[pos++] =
				ByteTools.readInt(data[i++], data[i++], data[i++], data[i++]);
		}
		data = null;
		return bbs;
	}
	...
	public final static void getBytes(Block[] outblocks, OutputStream out) {
		...
		for(int tx=0;tx<outblocks.length;tx++)blxt.add(outblocks[tx]);
		getBytes(blxt,out);
	}
...
//					if (LEOFile.DEBUG)Logger.logInfo("INFO: LEOFile.getBytes() getting bytes from Block: "+ i++	+ ":" + boc.getBlockIndex());
				...
				Logger.logWarn(
						"ERROR: gettting bytes from blocks failed: " + a);
			}
	...
	*/
    ...
    public byte[] getEncryptionStorageOverage() {
        if(encryptionStorageOverage==null)encryptionStorageOverage= new byte[0];
    ...
    private CompatibleVector getFATSectors() {
		...
		for (int x = 0; x<numExtraDIFATSectors; x++) {
			...
			for (int i = 0; i < numExtraSectorElements; i++) {
			    if (i<(IDXBLOCKSIZE-1)) {  // -1 because must always have room for end of sector mark 
