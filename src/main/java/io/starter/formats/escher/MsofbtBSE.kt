...
import io.starter.toolkit.MD4Digest


//0xF007
class MsofbtBSE(fbt: Int, inst: Int, version: Int) : EscherRecord(fbt, inst, version) {
     ...
     * @param cRef
     */
    var refCount = 1        // default reference count is 1; 0= hidden; > 1= multiple refs to same image   20071120 KSC

    override fun getData(): ByteArray {
        val imgHeader = ByteArray(61)    // BSE header = 36 bytes, BLIP record header = 24 bytes, then imageData bytes follow
        imgHeader[0] = imageType.toByte()   //btWin32
        imgHeader[1] = imageType.toByte()   //btMac

        // 20071004 KSC: Unknown type of record, does not follow below rules
        // and will cause file error ...
        // TODO: Research more
        if (imageType == 0) {
            val retData = ByteArray(36)
            retData[18] = (-1).toByte()    // tag
            length = retData.size
            return retData
        }
        //The digest of image data should be here. 2-17
        val md4Digest = MD4Digest()
        val digest = md4Digest.getDigest(imageData)
        System.arraycopy(digest, 0, imgHeader, 2, 16)

        imgHeader[18] = (-1).toByte()   //First tag byte is always -1
        imgHeader[19] = 0.toByte()    //Second tag byte is always 0
...
*/
        ...
        val sz = imageData!!.size + 25
        var tempBytes = ByteTools.cLongToLEBytes(sz)
        ...
        tempBytes = ByteTools.cLongToLEBytes(refCount)
        ...
        imgHeader[32] = 0.toByte()    //usage - should be 0=default usage
...
/*
		...
		*/
        ...
        var inst1 = MSODrawingConstants.msobiJPEG
when (imageType) {
MSODrawingConstants.IMAGE_TYPE_GIF -> {}
MSODrawingConstants.IMAGE_TYPE_EMF -> inst1 = MSODrawingConstants.msobiEMF
MSODrawingConstants.IMAGE_TYPE_WMF -> inst1 = MSODrawingConstants.msobiWMF
MSODrawingConstants.IMAGE_TYPE_PICT -> inst1 = MSODrawingConstants.msobiPICT
MSODrawingConstants.IMAGE_TYPE_JPG -> inst1 = MSODrawingConstants.msobiJPEG
MSODrawingConstants.IMAGE_TYPE_PNG -> inst1 = MSODrawingConstants.msobiPNG
MSODrawingConstants.IMAGE_TYPE_DIB -> inst1 = MSODrawingConstants.msobiDIB
}// TODO: ???
 //int inst1=(imageType==5)?MsodrawingConstants.msobiJPEG:MsodrawingConstants.msobiPNG;
        val version1 = 0
val fbt1 = MSODrawingConstants.msofbtBlipFirst + imageType
val len1 = imageData!!.size + 17
imgHeader[36] = (((0x0F and inst1) shl 4) or (0x0F and version1)).toByte()
imgHeader[37] = ((0xFF0 and inst1) shr 4).toByte()
imgHeader[38] = ((0xFF and fbt1)).toByte()
imgHeader[39] = ((0xFF00 and fbt1) shr 8).toByte()
...
imgHeader[60] = 0xff.toByte()   //marker to indicate start of image data
val len = 61
val retData = ByteArray(len + imageData!!.size)

System.arraycopy(imgHeader, 0, retData, 0, len)
System.arraycopy(imageData!!, 0, retData, len, imageData!!.size)

length = retData.size
return retData
}

 fun setImageData(d:ByteArray) {
imageData = d
}
 ...
 * serialVersionUID
 */
    private val serialVersionUID = -9072016434347371029L
}
