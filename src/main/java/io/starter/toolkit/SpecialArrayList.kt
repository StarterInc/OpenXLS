 ...
 * The <tt>add</tt> operation runs in *amortized constant time*, that is,
 ...
 *
 *
 * Each <tt>ArrayList</tt> instance has a *capacity*. The capacity is the
 ...
 * [ Java Collections
 * Framework]({@docRoot}/../guide/collections/index.html).
 *
 * @see Collection
 *
 * @see List
...
internal open class SpecialArrayList : AbstractList<Any>, List<Any>, RandomAccess, Cloneable, java.io.Serializable {
    ...
    constructor(c: Collection<*>) {
        ...
        elementData = arrayOfNulls(Math.min(size * 110L / 100, Integer.MAX_VALUE.toLong()).toInt())
        c.toTypedArray<Any>()
    }
    ...
    fun ensureCapacity(minCapacity: Int) {
        ...
        if (minCapacity > oldCapacity) {
            ...
            elementData = arrayOfNulls(newCapacity)
            System.arraycopy(oldData, 0, elementData, 0, size)
        }
    }
     ...
     * `false` otherwise.
     */
    override fun contains(elem: Any?): Boolean {
     ...
     * @see Object.equals
     */
    override fun indexOf(elem: Any?): Int {
        if (elem == null) {
     ...
     */
    override fun lastIndexOf(elem: Any?): Int {
        if (elem == null) {
    ...
    public override fun clone(): Any {
        try {
            val v = super.clone() as SpecialArrayList
            v.elementData = arrayOfNulls(size)
     ...
     * in determining the length of the list *only* if the caller knows that
     ...
     * @throws ArrayStoreException if the runtime type of a is not a supertype of the runtime type
     * of every element in this list.
     */
    override fun toArray(a: Array<Any>): Array<Any> {
        var a = a
        if (a.size < size)
            a = java.lang.reflect.Array.newInstance(a.javaClass.getComponentType(), size) as Array<Any>
    ...
    override fun addAll(c: Collection<*>): Boolean {
        val a = c.toTypedArray()
        // 20080124 KSC: replace with arraycopy
        // this.elementData = a;
        val numNew = a.size
        elementData = arrayOfNulls(numNew)
    ...
    override fun addAll(index: Int, c: Collection<*>): Boolean {
        ...
        val a = c.toTypedArray()
        val numNew = c.size // 20080121 KSC: a.length;
        ensureCapacity(size + numNew) // Increments modCount
    ...
    internal inner class Itr : Iterator<Any> {
        /**
        ...
        override fun remove() {
            ...
            try {
                this@SpecialArrayList.removeAt(lastRet)
                if (lastRet < cursor)
                    cursor--
                lastRet = -1
    ...
    companion object {
        private const val serialVersionUID = 8683452581122892189L
    }
}
/**
