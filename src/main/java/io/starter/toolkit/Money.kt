 ...
 *
 *
 * Money objects, like String objects, are **immutable**.
...
class Money // Money Class
    : Cloneable, // Money objects can be cloned
        Serializable // Money objects are serializable
{
     ...
     * formats.
     */
    protected var currencyFormat = NumberFormat.getCurrencyInstance() as DecimalFormat
    ...
    val isZero: Boolean
        get() = value!!.compareTo(ZERO) == 0 // Method Money.isZero()
    ...
    val isNegative: Boolean
        get() = value!!.compareTo(ZERO) < 0 // Method Money.isNegative()
    ...
    val isPositive: Boolean
        get() = value!!.compareTo(ZERO) >= 0 // Method Money.isPositive()
    // The value $0.00
     ...
     *
     */
    class InvalidScaleFactorException : RuntimeException // Non-checked exception
    {

        /**
         * Default constructor for a InvalidScaleFactorException object.
         */
        constructor() : super() {}// Invoke super class constructor.
         ...
         *
         */
        constructor(info: String) // Descriptive info
                : super(info) {
        ...
        companion object {
             ...
             * serialVersionUID
             */
            private val serialVersionUID = -8038085965896123803L
        }
    } // Class InvalidScaleFactorException
     ...
     */
    class InvalidRoundingModeException : RuntimeException // Non-checked exception
    {
        ...
        companion object {
             ...
             * serialVersionUID
             */
            private val serialVersionUID = 5658836125641516151L
        }
    } // Class InvalidRoundingModeException
     ...
     *
     */
    constructor(amount: Double) // Monetary Value, dollars and cents
    {
        ...
        value = BigDecimal(java.lang.Double.toString(amount))
     ...
     *
     *
     * @param **amount** The monetary amount, in whole
     * dollars (no cents)
     *
     */
    constructor(amount: Long) // Monetary amount, whole dollars (no cents)
    {
        value = BigDecimal(java.lang.Long.toString(amount)) // Set monetary value.
     ...
     * @param **amount** The monetary amount, in dollars
     * and cents
     * @param **scale**  Scale factor (must be 0, 1, or 2)
     *
     *
     *
     *
     ...
     */
    // Scale Factor (0, 1, 2)
    @Throws(InvalidScaleFactorException::class)
    constructor(amount: Long, // Montetary amount
                scale: Int) {
        if (// If the Scale Factor is not
                scale < 0 // 0, 1, or
                || scale > 2 // 2,
        )
            throw InvalidScaleFactorException(
                    "Invalid scale factor: $scale (must be 0, 1, or 2)")
        // Set the monetary value and scale as specified.
        value = BigDecimal(java.lang.Long.toString(amount)).movePointLeft(scale)
     ...
     *
     *
     * @param **string** A String representing a monetary
     * value
     *
     *
     *
    ...
    @Throws(ParseException::class) // If string is inconsistent with
    ...
    // Currency Format
    {
        ...
        // Convert the parsed value to a simple string (decimal point only)
        // and use it to set the monetary value.
        value = BigDecimal(number.toString())
     ...
     *
     *
     * @param **amount** A BigDecimal value representing a
     * monetary amount, in dollars and
     * cents
     *
     */
    constructor(amount: BigDecimal) // A BigDecimal object representing a
    // monetary amount
    {
        value = BigDecimal(amount.toString()) // Set the monetary value.
     ...
     *
     */
    constructor(amount: Money) // Money Object
    {
        roundingMode = amount.roundingMode // Copy the Rounding Mode.
        // Clone the Currency Format.
        currencyFormat = amount.currencyFormat.clone() as DecimalFormat
     ...
     *
     */
    fun add(money: Money) // The Monetary Value to be added
            : Money {
        val result = Money(this) // Create a new Money object for
        // the result.
     ...
     *
     */
    fun subtract(money: Money) // The monetary value to be
            : Money
    // subtracted
    {
        val result = Money(this) // Create a new Money object for
     ...
     *
     */
    fun multiply(mult: Double) // The multiplier value
            : Money {
        val result = Money(this) // Create a new Money object for
        // the result.
        // Multiply by the specified value.
        result.value = value!!.multiply(BigDecimal(mult))
        return result // Return the result.
     ...
     *
     */
    fun multiply(mult: Long) // The multiplier value
            : Money {
        val result = Money(this) // Create a new Money object for
        // the result.
        // Multiply by the specified value.
        result.value = value!!.multiply(BigDecimal(java.lang.Long.toString(mult)))
        return result // Return the result.
    } // Method Money.multiply()
     ...
     *
     */
    fun divide(div: Double) // The divisor value
            : Money {
        val result = Money(this) // Create a new Money object for
        // the result.
        // Divide the monetary value by the specified value and round if
        // necessary.
        result.value = value!!.divide(BigDecimal(div), BigDecimal.ROUND_HALF_UP)
        return result // Return the result.
    } // Method Money.divide()
     ...
     *
     */
    fun divide(div: Long) // The divisor value
            : Money {
        val result = Money(this) // Create a new Money object for
        // the result.
        // Divide the monetary value by the specified value and round if
        // necessary.
        result.value = value!!.divide(
                BigDecimal(java.lang.Long.toString(div)),
     ...
     */
    fun toDouble(): Double {
        return value!!.toDouble() // Return the monetary value as a
     ...
     */
    // Rounding Mode
    @Throws(InvalidRoundingModeException::class)
    fun setRoundingMode(mode: Int): Money {
        if (mode == BigDecimal.ROUND_UNNECESSARY)
        // If Rounding Mode is not
        // valid,
            throw InvalidRoundingModeException(
     ...
     * @see DecimalFormat
     */
    fun getCurrencyFormat(): DecimalFormat {
     ...
     * @see DecimalFormat
     */
    fun setCurrencyFormat(format: DecimalFormat) // Currency Format
            : Money {
        val result = Money(this) // Create a new Money object for
        // the result.
    ...
    override fun toString(): String {
        // Round off the monetary value to 2 decimal places.
        val result = value!!.setScale(2, roundingMode)
     ...
     * @see .setCurrencyFormat
     */
    // String representing a Monetary
    // Value
    @Throws(ParseException::class) // If string is inconsistent with
    fun parse(string: String): Money
    // Currency Format
    {
        val result = Money(this) // Create a Money object for the
        // result.
        // We make use of the format's parse() method, which parses the string
        // according to the format and returns either a Double or a Long
        // object representing the value.
        ...
        // Convert the parsed value to a simple string (decimal point only)
        // and use it to set the monetary value.
        result.value = BigDecimal(number.toString())
     ...
     *
     *
     * @param **other** A monetary value with which this
     ...
     *
     */
    fun isEqual(other: Money) // Monetary value for comparison
            : Boolean {
     ...
     *
     *
     * @return **true **  This monetary value is less than
     ...
     *
     */
    fun isLessThan(other: Money) // Monetary value for
            : Boolean
    // comparison
    {
     ...
     *
     */
    fun isLessThanOrEqual(other: Money) // Monetary value for
            : Boolean
    // comparison
    {
     ...
     *
     *
     * @return **true **  This monetary value is greater
     * than the specified monetary value
     * <br></br>
     ...
     *
     */
    fun isGreaterThan(other: Money) // Monetary value for
            : Boolean
    // comparison
    {
     ...
     *
     */
    fun isGreaterThanOrEqual(other: Money) // Monetary value for
            : Boolean
    // comparison
    {
     ...
     *
     */
    override fun equals(`object`: Any?) // Object to compare
            : Boolean {
        if (`object` === this)
        ...
        // definition.
        return if (`object` !is Money) false else value!!.compareTo(`object`.value!!) == 0 // the objects are not equal by
        // definition.
    ...
    public override fun clone(): Any {
        return Money(this) // Return the cloned object.
    }
    ...
    companion object {
         ...
         * serialVersionUID
         */
        private const val serialVersionUID = 2055249101317798370L
        // Currency format
        /**
         * The special monetary value of zero ($0.00).
         */
        protected val ZERO = BigDecimal("0.00")
    }
} // Class Money
