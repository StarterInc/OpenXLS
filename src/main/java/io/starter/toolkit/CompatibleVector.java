  ...
  */
    public int indexOf(CompatibleVectorHints r){
      //  return super.indexOf(r);
        ...
        x -= change_offset;
        if((x>0)&&(x<super.size()))if(super.elementAt(x).equals(r)){
        ...
        int t = -1;
        if(change_offset > reindex_change_size)this.resetHints(false);
        // if(x>0)t = super.indexOf(r,x);
		if(x>0)t = super.indexOf(r);
        if(t<0)t = super.indexOf(r);
...
/* */
 ...
 //       checkForComodification();
    ...
    public boolean add(CompatibleVectorHints obj){  
       this.change_offset++; // 
        int idx = super.size();
        if(obj != null)obj.setRecordIndexHint(idx);
        try{
